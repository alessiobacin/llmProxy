...
function createApp(options = {}) {
  ...
  if (!meteringSink) {
    ...
    } else if (meteringSinkMode === "db" || meteringSinkMode === "mongodb") {
      const mongoUri = process.env.LLMPROXY_MONGODB_URI;
      if (!mongoUri) throw new Error("LLMPROXY_MONGODB_URI is required when LLMPROXY_METERING_SINK=db");
      const { createMongoMeteringSink } = require("./metering-db");
      meteringSink = createMongoMeteringSink({
        uri: mongoUri,
        dbName: process.env.LLMPROXY_MONGODB_DB || "llmProxy",
        collectionName: process.env.LLMPROXY_MONGODB_METERING_COLLECTION || "metering",
      });
