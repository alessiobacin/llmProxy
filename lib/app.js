...
function createApp(options = {}) {
  ...
  // Codex CLI (models-manager) decodes `/v1/models` entries with a strict
  // schema: `slug`, `display_name`, `description`, `default_reasoning_level`,
  // `supported_reasoning_levels`, `shell_type`, `visibility`,
  // `supported_in_api`, `priority`, `additional_speed_tiers`, `service_tiers`,
  // `context_window`, `max_context_window`, `input_modalities`,
  // `supports_search_tool`, `use_responses_lite`, `tool_mode`,
  // `multi_agent_version`. Missing fields abort the refresh with
  // "failed to decode models response: missing field `X`". The `slug` field is
  // the model id; `id`/`object`/`created`/`owned_by` keep OpenAI compatibility.
