...
const { isVisionCapableModel } = require("./copilot-proxy");
const {
...
const { openAIRequestToAnthropic, anthropicResponseToOpenAI, anthropicErrorToOpenAI, createOpenAiStreamTranslator, anthropicSseWriteToOpenAiChunks } = require("./openai-format");
  const {
...
function createApp(options = {}) {
  ...
  // Codex CLI (models-manager) decodes `/v1/models` entries with the strict
  // `ModelInfo` schema from codex-rs/protocol/src/openai_models.rs. Every field
  // listed below is MANDATORY in Codex 0.152.x (no #[serde(default)] on the
  // struct): a single missing key aborts the refresh with "failed to decode
  // models response: missing field `X`". In particular, an empty `service_tiers`
  // silently hides the user's configured `service_tier = "priority"`, and a
  // `model_messages` without `instructions_template` makes Codex reject the
  // entire catalog. The `slug` field is the model id; `id`/`object`/`created`/
  // `owned_by` keep OpenAI compatibility.
  const codexModelDefaults = {
    ...
    // ServiceTier::Fast => "priority", ServiceTier::Flex => "flex". Advertising
    // at least "priority" keeps a user-level `service_tier = "priority"` valid;
    // `default_service_tier` stays absent (no forced default tier).
    service_tiers: [
      { id: "priority", name: "Fast", description: "Priority processing." },
      { id: "flex", name: "Flex", description: "Flexible on-demand processing." },
    ],
    ...
    // --- ModelInfo fields below are required by Codex >= 0.152 decoding ---
    truncation_policy: { mode: "tokens", limit: 120000 },
    availability_nux: null,
    upgrade: null,
    // ModelMessages: `instructions_template` must be a non-empty string or
    // Codex refuses the whole catalog ("missing both base_instructions and
    // model_messages.instructions_template"); the remaining ModelMessages
    // fields are explicit nulls so the decoder never trips on absent keys.
    model_messages: {
      instructions_template:
        "You are llmProxy, a local routing proxy. Follow the user's instructions and current Codex system guidance. You route through llmProxy which selects provider/model dynamically.",
  ...
  function buildCodexModelEntry(id, displayName, description, ownedBy, inputModalities) {
    return {
      slug: id,
      display_name: displayName,
      description,
      ...codexModelDefaults,
      input_modalities: inputModalities || codexModelDefaults.input_modalities,
    };
  }

  // Whether a configured provider entry can actually accept image input.
  // Mirrors the exact resolution order copilot-proxy.js uses at request time
  // (explicit `provider.vision` flag wins, else fall back to the per-model/
  // per-provider-kind heuristic) so `/v1/models` never advertises a
  // capability the router itself would reject.
  function resolveProviderVision(provider) {
    const explicitVision = provider.vision;
    if (explicitVision !== undefined && explicitVision !== null) return Boolean(explicitVision);
    const providerKind = String(provider.provider || provider.id || "unknown").trim().toLowerCase();
    return isVisionCapableModel(provider.default_model, providerKind);
  }

  function buildProxyAutoModel(providers) {
    // The auto-routing "llmproxy" model resolves its concrete provider/model
    // dynamically per request (including vision-aware fallback — see
    // copilot-proxy.js). It can accept images as long as at least one
    // configured provider is vision-capable; report that accurately instead
    // of a hardcoded text-only default, otherwise capability-aware clients
    // (Codex CLI, and others reading this same schema) will never even
    // attempt to send an image through it.
    const anyVisionCapable = Array.isArray(providers) && providers.some((provider) => resolveProviderVision(provider));
    return {
      id: "llmproxy",
      object: "model",
      ...
      ...buildCodexModelEntry(
        "llmproxy",
        "llmProxy Auto",
        "llmProxy virtual model: provider and concrete model are selected dynamically by the llmProxy router.",
        "llmproxy",
        anyVisionCapable ? ["text", "image"] : ["text"],
      ),
    };
  }
  ...
  function buildOpenAIModelEntry(provider, requestedId) {
    ...
    const id = requestedId || `${providerId}:${modelName}`;
    const inputModalities = resolveProviderVision(provider) ? ["text", "image"] : ["text"];
    return {
      id,
      object: "model",
      created: Math.floor(Date.now() / 1000),
      ...
      ...buildCodexModelEntry(
        ...
        `llmProxy routed model: ${modelName} via provider ${providerId}.`,
        providerId,
        inputModalities,
      ),
    };
  ...
  app.get("/v1/models", (_req, res) => {
    ...
    const seen = new Set();
    // `llmproxy` is the virtual model that deliberately leaves both provider
    // and concrete model selection to LLMProxy's dynamic router.
    const data = [buildProxyAutoModel(providers)];
    ...
    // `models` mirrors `data` for clients (e.g. Codex CLI models-manager) that
    // decode `{"models":[...]}` and abort with "missing field `models`" when
    // the key is absent. OpenAI-compatible clients keep reading `data`.
    res.json({ object: "list", data, models: data });
  });
  ...
  app.get("/v1/models/:modelId", (req, res) => {
    ...
    if (isProxyModelLabel(modelId)) return res.json(buildProxyAutoModel(providers));
  ...
  function isProxyModelLabel(value) {
    const normalized = String(value || "").trim().toLowerCase();
    // Accept exact matches: "llmproxy", "llm-proxy", "proxy-local"
    // Also accept "llmproxy auto", "llm-proxy auto", etc. (client IDE formats)
    const baseName = normalized.split(/\s+/)[0]; // Take first word if space-separated
    return baseName === "llmproxy"
      || baseName === "llm-proxy"
      || baseName === "proxy-local";
  }
  ...
  async function handleOpenAIChat(req, res, opts) {
    ...
    const captureRes = createOpenAICaptureResponse(res, traceId, {
      ...
      includeUsage: openAiBody.stream_options?.include_usage === true,
  ...
  async function handleResponses(req, res, opts) {
    ...
    const responsesBody = req.body || {};
    const openAiBody = responsesRequestToOpenAIChat(responsesBody);
    const anthropicBody = openAIRequestToAnthropic(openAiBody);
    if (responsesBody.provider) anthropicBody.provider = responsesBody.provider;
    if (responsesBody.shortAnswer !== undefined) anthropicBody.shortAnswer = responsesBody.shortAnswer;
    const captureRes = createResponsesCaptureResponse(res, traceId, {
      model: String(responsesBody.model || ""),
      stream: responsesBody.stream === true,
    });
    req.body = anthropicBody;
    await handleMessages(req, captureRes, { route: opts.route, enforceHierarchy: false, format: "responses" });
  }

  function createResponsesCaptureResponse(realRes, traceId, streamOptions = {}) {
    return {
      _status: 200,
      _headers: {},
      status(code) { this._status = code; return this; },
      setHeader(k, v) { this._headers[k] = v; realRes.setHeader(k, v); },
      getHeader(k) { return this._headers[k]; },
      json(payload) {
        if (this._status >= 400) {
          realRes.status(this._status).json(anthropicErrorToOpenAI(payload));
          return realRes;
        }
        const responses = anthropicResponseToResponses(payload, String(streamOptions.model || ""));
        realRes.status(this._status).json(responses);
        return realRes;
      },
      end() {
        if (this._responsesTranslator) {
          // Flush any buffered partial SSE event, then close the Responses stream.
          const tail = anthropicSseWriteToResponsesEvents(this._responsesTranslator, "\n\n");
          for (const chunk of tail) realRes.write(chunk);
        }
        realRes.end();
      },
      write(rawChunk) {
        // Streaming bridge: translate the Anthropic SSE events emitted by the
        // gateway into Responses API SSE events on the fly.
        if (!this._responsesTranslator) {
          this._responsesTranslator = createResponsesStreamTranslator({
            model: String(streamOptions.model || ""),
          });
        }
        const chunks = anthropicSseWriteToResponsesEvents(this._responsesTranslator, String(rawChunk ?? ""));
        for (const chunk of chunks) realRes.write(chunk);
        return realRes;
      },
    };
  }

  function createOpenAICaptureResponse(realRes, traceId, streamOptions = {}) {
    return {
      _status: 200,
      _headers: {},
      status(code) { this._status = code; return this; },
      ...
      json(payload) {
        ...
        if (this._openAiTranslator) {
          // Flush any buffered partial SSE event, then close the OpenAI stream.
          const tail = anthropicSseWriteToOpenAiChunks(this._openAiTranslator, "\n\n");
          for (const chunk of tail) realRes.write(chunk);
        }
        realRes.end();
      },
      write(rawChunk) {
        // Streaming bridge: translate the Anthropic SSE events emitted by the
        // gateway into OpenAI chat.completion.chunk payloads on the fly.
        if (!this._openAiTranslator) {
          this._openAiTranslator = createOpenAiStreamTranslator({
            model: String(streamOptions.model || ""),
            includeUsage: streamOptions.includeUsage === true,
          });
        }
        const chunks = anthropicSseWriteToOpenAiChunks(this._openAiTranslator, String(rawChunk ?? ""));
        for (const chunk of chunks) realRes.write(chunk);
        return realRes;
      },
    };
...
function registerLegacyGatewayRoutes(app, handlers) {
  ...
  app.post("/v1/responses", async (req, res) => {
    return handlers.handleResponses(req, res, { route: "/v1/responses", enforceHierarchy: false });
  });
}
...
function registerCanonicalGatewayRoutes(app, handlers) {
  ...
  app.post("/v1/llm/responses", async (req, res) => {
    return handlers.handleResponses(req, res, { route: "/v1/llm/responses", enforceHierarchy: true });
  });
}
