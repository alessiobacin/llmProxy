...
function createApp(options = {}) {
  ...
  const meteringSinkMode = process.env.LLMPROXY_METERING_SINK ||
    (mode === "platform" ? "jsonl" : "noop");
  const meteringSink = options.meteringSink || (
    meteringSinkMode === "jsonl"
      ? createJsonlMeteringSink({ filePath: paths.meteringFile })
   ...
   */
  app.get("/v1/llm/metering", (req, res) => {
    ...
    try {
      const result = meteringSink.query({ filters: params.filters, limit: params.limit, offset: params.offset, order: params.order });
   ...
   *   }
   */
  app.get("/v1/llm/metering/stats", (req, res) => {
    if (mode !== "platform" || typeof meteringSink.query !== "function") {
      return res.status(404).json({
        error: {
          code: "NOT_AVAILABLE",
    ...
    try {
      // Fetch all matching records (no pagination — stats need the full set)
      const result = meteringSink.query({ filters: params.filters, limit: 1_000_000, offset: 0, order: "asc" });
      const stats = computeMeteringStats(result.records);
      return res.json({ ...stats, filtered_total: result.total });
    } catch (err) {
      return res.status(500).json({ error: { code: "METERING_READ_ERROR", message: err.message } });
    }
  });
