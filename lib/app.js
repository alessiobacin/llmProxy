...
const TASK_STATUS_SYSTEM_TEXT = "At the start of every assistant reply, add one very short status line in the user's language describing what you are doing now, using 2-6 words. At the end of every final reply, explicitly state whether the task is completed. If completed, say so clearly in the user's language. If not completed, say it is not completed and briefly state what is still missing or blocked.";
...
function createApp(options = {}) {
  ...
  if (!meteringSink) {
    if (meteringSinkMode === "dblayer" && process.env.DBLAYER_URL) {
      ...
      meteringSink = createDbLayerSink({
        url: process.env.DBLAYER_URL,
        fallbackSink: localFallback,
        fetchFn: options.fetchFn || fetch,
        notifier: sendgridNotifier,
      });
    } else if (meteringSinkMode === "jsonl" || (meteringSinkMode === "dblayer" && !process.env.DBLAYER_URL)) {
  ...
  app.locals.reconfigureSinks = function (envOverrides = {}) {
    if ("DBLAYER_URL" in envOverrides) {
      ...
      if (newUrl && meteringSinkMode === "dblayer") {
        const localFallback = createJsonlMeteringSink({ filePath: paths.meteringFile });
        const { createDbLayerSink } = require("./metering-dblayer");
        ...
        sinkRefs.meteringSink = createNoopMeteringSink();
      }
    }
    ...
    if ("SENDGRID_API_KEY" in envOverrides || "SENDGRID_FROM_EMAIL" in envOverrides || "SENDGRID_TO_EMAIL" in envOverrides) {
      sinkRefs.sendgridNotifier.reconfigure({
        apiKey: envOverrides.SENDGRID_API_KEY,
        fromEmail: envOverrides.SENDGRID_FROM_EMAIL,
        toEmail: envOverrides.SENDGRID_TO_EMAIL,
      });
    }
  };
  ...
  app.post("/api/smart/add", async (req, res) => {
    const result = await executeCliCommand([
      "smart:add",
      "--provider", String(req.body?.provider || "").trim(),
      "--model", String(req.body?.model || "").trim(),
      "--api-key", String(req.body?.apiKey || req.body?.api_key || "").trim(),
    ]);
    const response = jsonFromCliResult(result, "smart:add");
    res.status(response.status).json(response.payload);
  });

  app.get("/api/smart/status", async (_req, res) => {
    const result = await executeCliCommand(["smart:status"]);
    const response = jsonFromCliResult(result, "smart:status");
    res.status(response.status).json(response.payload);
  });

  app.post("/api/smart/test", async (_req, res) => {
    const result = await executeCliCommand(["smart:test"]);
    const response = jsonFromCliResult(result, "smart:test");
    res.status(response.status).json(response.payload);
  });

  app.post("/api/smart/refresh", async (_req, res) => {
    const result = await executeCliCommand(["smart:refresh"]);
    const response = jsonFromCliResult(result, "smart:refresh");
    res.status(response.status).json(response.payload);
  });

  app.get("/api/config", async (req, res) => {
    const scope = String(req.query.scope || "").trim();
    const projectPath = String(req.query.projectPath || req.headers["x-project-path"] || "").trim();
  ...
  app.post("/api/config/:key", async (req, res) => {
    ...
    if (spec.hotReloadable && !spec.restartRequired) {
      if (["DBLAYER_URL", "EVENTBUS_URL", "SENDGRID_API_KEY", "SENDGRID_FROM_EMAIL", "SENDGRID_TO_EMAIL"].includes(key)) {
  ...
  app.delete("/api/config/:key", async (req, res) => {
    ...
    const projectPath = String(req.body?.projectPath || req.query.projectPath || req.headers["x-project-path"] || "").trim();
  ...
  async function handleMessages(req, res, opts) {
    ...
    canonicalBody.system = appendSystemInstruction(canonicalBody.system, TASK_STATUS_SYSTEM_TEXT);
    delete canonicalBody.provider;
    delete canonicalBody.shortAnswer;

    let smartRouteInfo = null;
    if (projectSettings.smartRoute && proxyControlsModel && !projectSettings.configuredModel) {
      try {
