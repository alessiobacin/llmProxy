...
function createApp(options = {}) {
  ...
  const sendgridNotifier = createSendGridNotifier({
    ...
    messageTypes: runtimeEnv.LLMPROXY_SENDGRID_TO_MESSAGE_TYPE || runtimeEnv.SENDGRID_TO_MESSAGE_TYPE,
  });
...
function startServer(options = {}) {
  ...
  const benchmarkMinutes = resolveBenchmarkMinutes();
  if (benchmarkMinutes > 0 && options.tokenStore) {
    try {
      const probeEndpoint = (() => {
        // Minimal endpoint map for benchmark probe requests
        ...
        return (provider) => {
          ...
          if (provider.proxy_url) return provider.proxy_url.replace(/\/+$/, "") + "/v1/chat/completions";
          if (pType === "copilot") return "https://api.githubcopilot.com/chat/completions";
          return PROBE_ENDPOINTS[pType] || null;
        };
      })();
      const benchmark = createProviderBenchmark({
        filePath: path.join(options.dataRoot || paths?.dataRoot || require("./paths").createPaths().dataRoot, "provider-benchmark.json"),
        fetchFn: options.fetchFn || fetch,
        probeFn: async ({ provider, model, fetchFn }) => {
          const url = probeEndpoint(provider);
          if (!url) return { ok: false };
          const pType = (provider.provider || "").toLowerCase();
          const isAnthropic = pType === "anthropic" || pType === "opencode-go";
          const body = isAnthropic
            ? JSON.stringify({ model: model || "claude-sonnet-4-20250514", max_tokens: 1, messages: [{ role: "user", content: "hi" }] })
            : JSON.stringify({ model: model || "gpt-4o-mini", max_tokens: 1, messages: [{ role: "user", content: "hi" }] });
      ...
      const providers = (options.tokenStore.listProviders || (() => []))().filter((p) => p && p.access_token);
      benchmark.start(providers, benchmarkMinutes);
      options.tokenStore.__llmproxyProviderBenchmark = benchmark;
    } catch {
      // best effort: benchmark is non-critical
    }
  }
