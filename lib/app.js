...
function createApp(options = {}) {
  ...
  // Enabled only when LLMPROXY_API_KEY is set; health/logout stay open.
  const inboundApiKey = String(runtimeEnv.LLMPROXY_API_KEY || "").trim();
  if (inboundApiKey) {
    const PUBLIC_PATHS = new Set(["/health", "/auth/logout", "/v1/llm/health"]);
    app.use((req, res, next) => {
      ...
      res.status(401).json({ error: "UNAUTHORIZED", message: "API key mancante o non valida." });
    });
  }
  ...
  app.get("/v1/models/:modelId", (req, res) => {
    ...
    for (const provider of providers) {
      if (String(provider.default_model || "").trim() === modelId) {
        return res.json({
          id: modelId,
          object: "model",
  ...
  async function handleMessages(req, res, opts) {
    ...
    // Inject intent prompt when escalation is active
    if (Number(runtimeEnv.LLMPROXY_INTENT_ESCALATION || 0) > 0) {
