...
const COMMAND_HELP = {
  ...
  "provider:add": {
    usage: "llmproxy provider:add <id> [--name <name>] [--api-key <key>] [--model <model>]",
    ...
    example: "llmproxy provider:add openrouter --api-key sk-or-key --model openai/gpt-4o",
  },
  "provider:key": {
    usage: "llmproxy provider:key <id> --api-key <key>",
    ...
    example: "llmproxy provider:key openrouter --api-key sk-new-key",
  },
  "provider:available": {
...
function resolveUpdatePrerequisites(options = {}) {
  ...
  const runtimeEntry = resolveServiceEntryFile({ env, packageRoot });
...
function resolveCliServiceManagerOptions({ env, paths, targetPlatform }) {
  ...
  return {
    ...
    entryFile: resolveServiceEntryFile({ env, packageRoot: paths.packageRoot }),
...
function resolveServiceEntryFile({ env = process.env, packageRoot }) {
