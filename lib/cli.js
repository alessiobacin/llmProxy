...
const COMMAND_HELP = {
  ...
  "provider:test": {
    ...
    description: "Testa la capacita' di visione di tutti i provider configurati. Con --all-proxies testa ogni combinazione provider × proxy. Con --proxy <url> forza un proxy specifico. Con --provider <id> testa solo un provider.",
...
Provider:
  ...
  llmproxy provider:test [--all-proxies]   ${s("provider:test")}  testa la capacita' di visione; --all-proxies testa ogni proxy
...
Proxy (registro e rotazione):
  ...
  llmproxy provider:test --all-proxies     ${s("provider:test")}  testa i provider attraverso tutti i proxy
...
function formatProviderList(providers, options = {}) {
  ...
  return providers.map((provider, index) => {
    ...
    const details = [
      ...
      visionSuffix ? `   ${visionSuffix.trim()}` : null,
      `   price=${currentPrice}`,
...
async function runCli(argv, options = {}) {
          ...
          `${proxyBaseUrl}/v1/messages`,
          {
            ...
            body: JSON.stringify({
              ...
              messages: [
                {
                  role: "user",
                  content: [{ type: "text", text: "Rispondi solo: ciao" }],
                },
              ],
            }),
          },
  ...
  if (parsed.command === "provider:test") {
    ...
    let proxiesList = [];
    if (forcedProxyUrl) {
      stdout.write(`Test provider con proxy forzato: ${forcedProxyUrl}\n\n`);
      ...
      stdout.write("Test provider con rotazione su tutti i proxy...\n\n");
    } else {
    ...
    for (const provider of providers) {
      ...
      // In --all-proxies mode testa ogni proxy, altrimenti testa diretto
      const proxyUrls = allProxiesMode ? proxiesList.map((p) => p.url) : [""];
      ...
      for (const proxyUrl of proxyUrls) {
        ...
        try {
          ...
          const testFetch = proxyUrl ? makeProxyFetch(fetchFn, proxyUrl) : fetchFn;
          const response = await runLocalProxyTestRequest(
            testFetch,
            `${baseUrl}/v1/messages`,
            {
        ...
        if (isProxyError && allProxiesMode) {
          stdout.write(`  ℹ️  Proxy non raggiungibile (ignorato in modalità probe)\n`);
