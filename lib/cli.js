...
const COMMAND_HELP = {
  ...
  "config:get": {
    ...
    example: "llmproxy config:get LLMPROXY_SMART_ROUTE --project",
  },
  "config:set": {
    ...
    example: "llmproxy config:set LLMPROXY_SMART_ROUTE hybrid --project",
  },
  "config:unset": {
...
Comandi principali:
  ...
  llmproxy uninstall                       disinstallazione completa: servizio, Docker, dati e pacchetto globale

Autenticazione:
  llmproxy login                           alias legacy di provider:add copilot; mantienilo solo per compatibilita'
  llmproxy logout                          rimuove i token locali; usalo per resettare l'accesso corrente

Esecuzione proxy:
  llmproxy run                             avvia il proxy in foreground su http://127.0.0.1:5045 in sviluppo locale
  ...
  llmproxy stats:reset [--hard]             azzera le statistiche di utilizzo; con --hard resetta anche cache e auto-rank
...
Provider:
  ...
  llmproxy provider:key <id> --api-key <key> aggiorna/imposta la API key per provider API-key (openrouter, kimi, qwen, z.ai, opencode, ...)
  llmproxy provider:available              elenca i provider supportati dalla CLI
  llmproxy provider:list                   elenca i provider configurati o la chain effettiva del progetto corrente
  ...
  llmproxy provider:usage                  mostra consumo token giornaliero, settimanale e mensile
...
Modelli e Claude Code:
  ...
  llmproxy config:list [--project|--service] mostra le variabili configurabili gestite da llmproxy
  llmproxy config:get <key> [--project|--service] legge una variabile configurabile
  llmproxy config:set <key> <value> [--project|--service] imposta una variabile configurabile
  llmproxy config:unset <key> [--project|--service] rimuove una variabile configurabile
...
Quando usare cosa:
  1. Prima installazione persistente: install:persistent-it oppure install:persistent-en -> provider:add copilot -> models:list -> claude:setup
...
Problemi comuni:
  ...
  - il token Copilot non funziona: esegui llmproxy logout e poi llmproxy provider:add copilot
`);
}
...
function resolvePersistentInstallPrerequisites(options = {}) {
  ...
  if (targetPlatform === "linux" && commandIsAvailable(commandRunner, "systemctl", ["--version"])) {
    // Under sudo, systemctl --user needs the real user's XDG_RUNTIME_DIR and D-Bus address
    const sudoUser = process.env.SUDO_USER;
    const sudoUid = (() => { try { return spawnSync("id", ["-u", sudoUser], { encoding: "utf8" }).stdout.trim(); } catch { return null; } })();
    const runtimeDir = sudoUid ? `/run/user/${sudoUid}` : (process.env.XDG_RUNTIME_DIR || "");
    const dbusAddr = runtimeDir ? `unix:path=${runtimeDir}/bus` : "";
    const systemctlEnv = runtimeDir ? `XDG_RUNTIME_DIR=${runtimeDir} DBUS_SESSION_BUS_ADDRESS=${dbusAddr}` : "";
    const checkCmd = systemctlEnv
      ? `env ${systemctlEnv} systemctl --user is-system-running --quiet`
...
function formatProviderList(providers, options = {}) {
  ...
  return providers.map((provider, index) => {
    ...
    const modelSuffix = defaultModel
      ? ` model=${colorize(defaultModel, "yellow", useColor)}`
      : ` model=${colorize("missing", "red", useColor)}`;

    let creditSuffix = "";
    let inlineCredit = "";

    if (creditInline) {
      // Mostra il credito inline subito dopo il nome del provider
      const creditLabel = String(provider.credit_info?.label || "n/a").trim() || "n/a";
      const creditColor = creditLabel === "n/a"
        ? "dim"
        : creditLabel === "unavailable"
          ? "red"
          : creditLabel.includes("-")
            ? "red"
            : "blue";
      inlineCredit = ` [${colorize(creditLabel, creditColor, useColor)}]`;
    } else {
      // Comportamento originale: credito alla fine
      creditSuffix = formatProviderCreditSuffix(provider.credit_info, useColor);
    }

    return `${linePrefix} ${providerId} (${providerName})${inlineCredit}${modelSuffix}${formatVisionSuffix(provider, useColor)}${formatProviderPlanSuffix(provider, useColor)}${creditSuffix}${formatProviderPriceSuffix(provider.price_info, useColor)}`;
  }).join("\n");
}
...
async function fetchProviderCreditInfo(provider, fetchFn, cache = new Map()) {
  ...
  if (!providerKind || !accessToken) return { label: "n/a" };
  ...
  const promise = (async () => {
    ...
    try {
      ...
      if (providerKind === "openrouter") {
        ...
        return { label: `${remainingCredits.toFixed(2)} credits` };
      }
...
async function fetchProviderPriceInfo(provider, fetchFn, cache = new Map()) {
  ...
  if (!model) return { currentPriceLabel: "n/a", bestProviderLabel: "n/a", bestPriceLabel: "n/a" };
  const data = await fetchCloudPriceModelPricing(model, fetchFn, cache);
  if (!data) return { currentPriceLabel: "unavailable", bestProviderLabel: "unavailable", bestPriceLabel: "unavailable" };
  ...
  return {
    currentPriceLabel: currentOption ? formatCloudPriceBreakdownLabel(currentOption) : "n/a",
    bestProviderLabel: bestProvider ? mapCloudPriceProviderLabel(bestProvider.provider_id) : "n/a",
    bestPriceLabel: data?.result ? formatCloudPriceBreakdownLabel(data.result) : "n/a",
  };
}
...
function runSelfUpdate(commandRunner = spawnSync, repo = "alessiobacin/llmProxy", locale = "it") {
  const script = [
    ...
    '          echo "Error: Cannot install llmproxy globally."',
    '          exit 1',
    '        fi',
    '      fi',
    '    else',
...
function runSelfUpdateWindows(commandRunner = spawnSync, repo = "alessiobacin/llmProxy", locale = "it") {
  const psScript = [
    ...
    '',
    '# Output results',
    '$versionOutput = & "$newBin" version 2>$null',
    ...
    'Write-Output "__LLMPROXY_VERSION__=$targetVersion"',
...
function buildPersistentInstallScript(options = {}) {
  ...
  return [
    ...
    'existing_global_bin=""',
    'for candidate in $(which -a llmproxy 2>/dev/null | awk \'!seen[$0]++\'); do',
    '  case "$candidate" in',
    '    "$HOME"/*) ;;',
    ...
    'case "$npm_prefix_global" in',
    '  "$HOME"/*)',
    '    # User-local npm prefix: try system-wide install via sudo for multi-user server',
    '    if command -v sudo >/dev/null 2>&1; then',
    `      if sudo npm install -g ${shellQuote(packageRoot)} 2>/dev/null; then`,
    '        used_sudo=1',
    '        echo "System-wide install via sudo (npm prefix was user-local)."',
    '      else',
    '        echo "Warning: could not install system-wide. Falling back to user-local." >&2',
    ...
    '      fi',
    '    else',
    `      # No sudo — install user-local only`,
    `      if ! npm install -g ${shellQuote(packageRoot)} 2>/dev/null; then`,
    ...
    '  sudo -u "$SUDO_USER" XDG_RUNTIME_DIR="/run/user/$(id -u "$SUDO_USER")" DBUS_SESSION_BUS_ADDRESS="unix:path=/run/user/$(id -u "$SUDO_USER")/bus" "$global_bin" service:start',
    'else',
...
function writeClaudeSettings(options = {}) {
  ...
  if (proxyEnv.ANTHROPIC_AUTH_TOKEN) {
...
function syncClaudeHomeSettings(options = {}) {
  ...
  fs.mkdirSync(settingsDir, { recursive: true });
  fs.writeFileSync(settingsFile, `${JSON.stringify(nextConfig, null, 2)}\n`, "utf8");
  stdout.write(`Configurazione Claude globale aggiornata in ${settingsFile}\n`);
}
...
function buildRestCommandRequest(parsed, options = {}) {
  ...
  switch (command) {
    ...
    case "help":
      if (parsed.args[0]) query.set("command", parsed.args[0]);
    ...
    case "smart:add":
      return {
        ...
        headers,
        body: {
          provider: String(parsed.flags.provider || "").trim(),
          model: String(parsed.flags.model || "").trim(),
          apiKey: String(parsed.flags["api-key"] || "").trim(),
        },
      };
    case "smart:status":
      return { method: "GET", path: "/api/smart/status" };
    case "smart:test":
      return { method: "POST", path: "/api/smart/test" };
    case "smart:refresh":
      return { method: "POST", path: "/api/smart/refresh" };
    case "update":
...
async function tryRunViaRest(parsed, options = {}) {
  ...
  if (options.tokenStore || options.modelCatalogStore || options.serviceManager || options.commandRunner || options.smartRouterStore) {
    return null;
  }
  let request = null;
  try {
...
async function runCli(argv, options = {}) {
  ...
  if (parsed.command === "test") {
    ...
    if (inferenceMode) {
      ...
      try {
        ...
        const summary = readLatestRequestSummary(paths.logsDir, startedAtIso);
        ...
        if (allProviders) {
          ...
          remainingFallbackProviders.forEach((provider, index) => {
            const model = normalizeInferenceDisplayModel(provider.effective_model || provider.default_model || (provider.provider === "copilot" ? DEFAULT_COPILOT_MODEL : ""));
            stdout.write(`${formatOrdinalLabel(index + 1)} fallback: ${provider.id} | ${model || "missing"}\n`);
          });
        }
    ...
    for (const provider of selectedTargets) {
      ...
      try {
        ...
        stdout.write(`${provider.id}: ok (${model})\n`);
      } catch (error) {
        const errorMessage = error.message || String(error || "");
  ...
  if (parsed.command === "provider:add") {
    ...
    if (providerInfo.auth === "api_key") {
      ...
      stdout.write(`Provider configurato con API key: ${instanceId} (default model: ${defaultModel}, vision: ${visionEnabled}${planSuffix}).\n`);
      return 0;
    }
  ...
  if (parsed.command === "provider:key") {
    ...
    stdout.write(`API key aggiornata per provider ${effectiveId} (default model: ${nextDefaultModel}${visionSuffix}${planSuffix}).\n`);
    return 0;
  }
  ...
  if (parsed.command === "stats:reset") {
    try {
      ...
      // 2) Smart router cache (solo --hard)
      if (parsed.flags.hard && fs.existsSync(paths.smartRouterFile)) {
        fs.writeFileSync(paths.smartRouterFile, JSON.stringify({}, null, 2), "utf8");
        truncated.push("smart-router");
      }
