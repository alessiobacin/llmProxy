...
async function runCli(argv, options = {}) {
  ...
  if (parsed.command === "provider:add") {
    ...
    if (providerInfo.auth === "api_key") {
      ...
      const existingWithSameModel = hasExplicitInstanceId ? null : sameKindProviders.find((p) => p.default_model === defaultModel);
      const targetInstanceId = existingWithSameModel
        ? existingWithSameModel.id
        : (hasExplicitInstanceId ? providerId : (sameKindProviders.length > 0 ? defaultModel : providerKind));
