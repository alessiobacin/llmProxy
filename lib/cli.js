  ...
  if (parsed.command === "test") {
    ...
    if (inferenceMode) {
      ...
      try {
        ...
        if (allProviders && Array.isArray(summary?.providerSequence) && summary.providerSequence.length > 0) {
          stdout.write(`fallback: ${summary.providerSequence.map(formatInferenceProviderAttempt).join(" -> ")}\n`);
        }
        stdout.write(`response: ${visibleAssistantText}\n`);
        return 0;
      } catch (error) {
