...
function printCommandHelp(command, stdout = process.stdout) {
  ...
  stdout.write(`Descrizione: ${entry.description}\n`);
  stdout.write(`Quando usarlo: ${entry.when}\n`);
  if (entry.example) stdout.write(`Esempio: ${entry.example}\n`);
  return 0;
}
...
Comandi principali:
  ...
  llmproxy install:persistent              installa globalmente la CLI corrente e attiva il servizio persistente nativo dell'OS
...
Quando usare cosa:
  1. Prima installazione persistente: install:persistent -> login -> models:list -> claude:setup
...
async function runCli(argv, options = {}) {
  ...
  if (parsed.command === "install:persistent") {
