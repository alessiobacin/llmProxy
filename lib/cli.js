...
const { loadRuntimeEnv, normalizeRuntimeProfile } = require("./runtime-env");
...
function formatReleaseNotes(version, locale = "it") {
...
function runSelfUpdate(commandRunner = spawnSync, repo = "alessiobacin/llmProxy", locale = "it") {
  const script = [
    ...
    `release_notes_output=$("$new_bin" release-notes --version "$version_output" --locale ${shellQuote(normalizeLocale(locale))})`,
...
async function runCli(argv, options = {}) {
  ...
  const serviceManager = options.serviceManager || createServiceManager({
    ...
    environment: resolveServiceEnvironment({ env, paths, dockerComposeFile }),
  });
  ...
  if (parsed.command === "release-notes") {
    ...
    stdout.write(`${formatReleaseNotes(version, locale)}\n`);
    return 0;
  }
  ...
  if (parsed.command === "service:restart") {
    ...
    if (!result.ok) {
      stderr.write((result.stderr && String(result.stderr).trim()) || "Riavvio servizio fallito.\n");
      return 1;
    }
