...
function formatProviderList(providers, options = {}) {
  ...
  const env = options.env || process.env;
  const projectCreditInline = options.creditInline === true || options.creditInline === false
    ? options.creditInline
    : null;
  const creditInline = projectCreditInline !== null
    ? projectCreditInline
    : String(env?.LLMPROXY_PROVIDER_CREDIT_INLINE || "").trim() === "1";
  return providers.map((provider, index) => {
    ...
    const proxyValue = provider.proxy_url
      ? colorize(provider.proxy_url, "magenta", useColor)
      : null;
  ...
  if (parsed.command === "test") {
    ...
    if (targetProviderArg) {
      ...
      if (numericIndex >= 1 && numericIndex <= providers.length) {
        ...
        targetProvider = providers.find((p) => String(p.id || "").toLowerCase() === targetProviderArg.toLowerCase()) || null;
      }
      if (!targetProvider) {
        stderr.write(`Provider non trovato: ${targetProviderArg}\nUsa llmproxy provider:list per vedere i provider configurati.\n`);
        return 1;
      }
    }
