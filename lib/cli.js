...
function resolveVscodeChatModelsFile(env = process.env) {
  ...
  if (platform === "darwin") {
    return path.join(homeDir, "Library", "Application Support", "Code", "User", "chatLanguageModels.json");
  }
...
function resolveModelsFromTokenStore(tokenStore) {
  ...
  for (const provider of providers) {
    const modelId = String(provider.default_model || "").trim();
    ...
    models.push({
      ...
      object: "model",
      created: now,
      owned_by: String(provider.provider || provider.id || "unknown").trim().toLowerCase(),
    });
  }
  return models;
}

function writeVscodeChatSettings(options = {}) {
  const env = options.env || process.env;
  const stdout = options.stdout || process.stdout;
  const modelsFile = resolveVscodeChatModelsFile(env);
  const modelsDir = path.dirname(modelsFile);
  ...
  const models = options.availableModels && Array.isArray(options.availableModels)
    ? options.availableModels
    : resolveModelsFromTokenStore(options.tokenStore);
  ...
  stdout.write(`  Modelli: ${models.length} (${Object.keys(modelEntries).join(", ")})\n`);
...
async function runCli(argv, options = {}) {
  ...
  if (parsed.command === "vscode-chat:setup") {
    try {
      const models = resolveModelsFromTokenStore(tokenStore);
