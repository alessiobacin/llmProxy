...
function runSelfUpdate(commandRunner = spawnSync, repo = "alessiobacin/llmProxy", locale = "it") {
  const script = [
    ...
    'pnpm remove -g @alessiobacin/llmproxy >/dev/null 2>&1 || true',
...
function runSelfUninstall(commandRunner = spawnSync) {
  const script = [
    'set -e',
    'npm uninstall -g @alessiobacin/llmproxy >/dev/null 2>&1 || true',
    'pnpm remove -g @alessiobacin/llmproxy >/dev/null 2>&1 || true',
    'pnpm_root=$(pnpm root -g 2>/dev/null || true)',
    'if [ -n "$pnpm_root" ]; then',
    '  pnpm_home=$(dirname "$(dirname "$pnpm_root")")',
...
function buildWindowsPersistentInstallScript(options = {}) {
  ...
  const lines = locale === "en"
    ? [
      ...
      "npm uninstall -g @alessiobacin/llmproxy 2>$null | Out-Null",
      "pnpm remove -g @alessiobacin/llmproxy 2>$null | Out-Null",
      "# Install globally",
...
async function runCli(argv, options = {}) {
  ...
  if (parsed.command === "uninstall") {
    ...
    const result = runSelfUninstall(commandRunner);
    if (result?.status === 0) {
