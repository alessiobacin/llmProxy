...
Usage:
  ...
  llmproxy claude:setup [--model <indice>]
  llmproxy version
  llmproxy update
  llmproxy setup
`);
...
async function runCli(argv, options = {}) {
  ...
  if (parsed.command === "update") {
    ...
    if (result?.status === 0) {
      ...
      if (result.stdout) stdout.write(`${String(result.stdout).trim()}\n`);
      const versionCheck = (options.commandRunner || spawnSync)("llmproxy", ["version"], { encoding: "utf8" });
      if (versionCheck?.status === 0) {
        const version = String(versionCheck.stdout || "").trim();
        if (version) stdout.write(`Versione corrente: ${version}\n`);
        return 0;
      }
      stderr.write((versionCheck?.stderr && String(versionCheck.stderr).trim()) || "Aggiornamento completato, ma il rilancio di llmproxy e' fallito.\n");
      return 1;
    }
