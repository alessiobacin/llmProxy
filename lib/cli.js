...
function buildLocalScopeToken(env = process.env, scopeUser) {
  const userName = String(scopeUser || resolveScopedProviderUser(env, os) || "").trim();
  if (!userName) return "";
  const secret = String(env.LLMPROXY_SECRET || "").trim();
  if (!secret) return `llmproxy-local-user:${userName}`;
  const signature = crypto.createHmac("sha256", secret).update(userName).digest("hex");
  return `llmproxy-local-user:${userName}:${signature}`;
}
...
function providerMatchesModelPreference(provider, preferenceProvider) {
  const normalizedPreference = String(preferenceProvider || "").trim().toLowerCase();
  if (!normalizedPreference) return true;
  return normalizedPreference === getProviderKind(provider)
    || normalizedPreference === String(provider?.id || "").trim().toLowerCase();
}
...
function resolveEffectiveProviderList(providers, configuredModel) {
  ...
  const effectiveProviders = baseProviders.map((provider) => {
    const defaultModel = provider.default_model || (getProviderKind(provider) === "copilot" ? DEFAULT_COPILOT_MODEL : "");
    const matchingPreferences = preferences.filter((preference) => providerMatchesModelPreference(provider, preference.provider));
    const effectiveModel = matchingPreferences[0]?.model || defaultModel;
    return {
      ...provider,
...
function formatProviderList(providers) {
  return providers.map((provider, index) => {
    ...
    return `${index + 1}. ${provider.id} (${provider.name})${modelSuffix}${formatVisionSuffix(provider)}${formatProviderPlanSuffix(provider)}`;
  }).join("\n");
}
...
function writeClaudeSettings(options = {}) {
  ...
  if (isSharedProviderRegistryEnabled(env)) {
    const localScopeToken = buildLocalScopeToken(env, options.scopeUser);
    if (localScopeToken) {
      proxyEnv.ANTHROPIC_AUTH_TOKEN = localScopeToken;
    }
  }
  ...
  if (options.omitAuthToken !== false && !isSharedProviderRegistryEnabled(env)) {
...
function isDockerServiceRunning(commandRunner, composeFile, serviceName) {
  const result = runCommand(commandRunner, "docker", ["compose", "-f", composeFile, "ps", "--status", "running", "--services", serviceName]);
...
function ensureDockerServiceRunning({ commandRunner, env, paths, targetPlatform }) {
  ...
  if (isDockerServiceRunning(commandRunner, composeFile, serviceName)) {
    return { ok: true, checked: true, restarted: false };
  }
  const upResult = runCommand(commandRunner, "docker", ["compose", "-f", composeFile, "up", "-d", "--build", serviceName]);
  if (!upResult || upResult.status !== 0) {
    return {
      ok: false,
      error: String(upResult?.stderr || upResult?.stdout || "docker compose up failed").trim(),
    };
  }
  if (!isDockerServiceRunning(commandRunner, composeFile, serviceName)) {
    return { ok: false, error: `Il container Docker ${serviceName} non risulta attivo dopo il restart.` };
  }
  return { ok: true, checked: true, restarted: true };
}
...
async function runCli(argv, options = {}) {
  ...
  const providerStore = isSharedProviderRegistryEnabled(env)
    ? createSharedProviderStore({
      providerRegistry,
      scopeUser: resolveScopedProviderUser(env, os),
      providerNameResolver(providerId) {
        return getKnownProvider(providerId).displayName || String(providerId || "").trim();
      },
    })
    : tokenStore;
        ...
        const response = await runLocalProxyTestRequest(
          ...
          `${proxyBaseUrl}/v1/messages`,
          {
            ...
            headers: {
              ...
              ...(isSharedProviderRegistryEnabled(env)
                ? { "x-api-key": buildLocalScopeToken(env, providerStore.scopeUser) }
                : {}),
            },
            body: JSON.stringify({
  ...
  if (parsed.command === "provider:add") {
    ...
    if (providerInfo.auth === "api_key") {
      ...
      providerStore.saveProvider(providerId, {
        access_token: apiKey,
        token_type: "api_key",
        scope: "api_key",
        provider: providerId,
  ...
  if (parsed.command === "provider:list") {
    ...
    stdout.write(`${formatProviderList(effectiveProviders.providers)}\n`);
    return 0;
  }
  ...
  if (parsed.command === "status") {
    ...
    stdout.write(`Authenticated: ${providerStore.getAccessToken() ? "yes" : "no"}\n`);
    if (providers.length > 0) {
