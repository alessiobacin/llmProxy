...
function formatProviderList(providers, options = {}) {
  ...
  return providers.map((provider, index) => {
    ...
    const details = [
      `   model=${modelValue}`,
      `   credit=${creditValue}`,
      `   coding=${codingValue}${formatVisionSuffix(provider, useColor)}${formatProviderPlanSuffix(provider, useColor)}`,
      `   price=${currentPrice}`,
  ...
  if (parsed.command === "provider:list") {
    ...
    const providersWithCredit = await Promise.all(
      effectiveProviders.providers.map(async (provider) => {
        ...
        if (benchMetric === "n/a") {
          const speedMs = await fetchSpeedMetric(provider, probeFn, fetchFn);
          benchMetric = speedMs != null ? `${speedMs} ms` : "n/a";
        }
