...
function runSelfUpdate(commandRunner = spawnSync, repo = "alessiobacin/llmProxy", locale = "it") {
  const script = [
    ...
    '    return 0',
    '  fi',
    '  target_dir=$(dirname "$global_bin_path")',
    '  if [ "$new_bin_mode" = "node" ] && [ -f "$package_cli" ]; then',
    '    wrapper_payload=$(printf \'%s\\n\' \'#!/bin/sh\' "exec node \\"$package_cli\\" \\"\\$@\\"" )',
    '  elif [ -n "$new_bin" ] && [ -e "$new_bin" ]; then',
    '    wrapper_payload=$(printf \'%s\\n\' \'#!/bin/sh\' "exec \\"$new_bin\\" \\"\\$@\\"" )',
    '  else',
    '    return 1',
    '  fi',
    '  if [ -w "$target_dir" ]; then',
    '    printf "%s\\n" "$wrapper_payload" > "$global_bin_path"',
    '    chmod +x "$global_bin_path"',
    '    return 0',
    '  fi',
    '  if command -v sudo >/dev/null 2>&1; then',
    '    printf "%s\\n" "$wrapper_payload" | sudo tee "$global_bin_path" >/dev/null',
    '    sudo chmod +x "$global_bin_path"',
    '    return 0',
    '  fi',
    '  return 1',
    '}',
    ...
    '  fi',
    'done',
    'if [ -z "$new_bin" ] && [ -x "$global_bin_path" ]; then',
