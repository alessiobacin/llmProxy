...
function extractStatsModels(payload) {
...
function extractCodingScore(entry) {
  const candidates = [
    entry?.coding,
    entry?.coding_score,
    entry?.codingScore,
    entry?.top_scores?.code,
    entry?.topScores?.code,
  ];
...
function findStatsModelEntry(models, model) {
  const candidates = new Set(normalizeModelLookupCandidates(model));
  if (candidates.size === 0) return null;
  for (const entry of Array.isArray(models) ? models : []) {
    const entryCandidates = normalizeModelLookupCandidates(entry?.model || entry?.id || entry?.name || entry?.slug || "");
    if (entryCandidates.some((candidate) => candidates.has(candidate))) return entry;
  }
  return null;
}

async function fetchProviderCodingInfo(provider, fetchFn, env = process.env, cache = new Map()) {
  const model = String(provider?.effective_model || provider?.default_model || "").trim();
  const apiKey = String(env?.LLM_STATS_API_KEY || "").trim();
  if (!model || !apiKey) return { label: "n/a", color: "dim" };

  const cacheKey = apiKey;
  if (cache.has(cacheKey)) {
    const models = await cache.get(cacheKey);
    const entry = findStatsModelEntry(models, model);
    const label = formatCodingScoreLabel(extractCodingScore(entry));
    return { label, color: colorForCodingScore(label) };
  }

  const promise = (async () => {
    try {
      const response = await fetchFn("https://api.zeroeval.com/stats/v1/models", {
        method: "GET",
        headers: {
      ...
      return extractStatsModels(payload);
    } catch {
      return null;
    }
  })();
  ...
  const entry = findStatsModelEntry(models, model);
  const label = models ? formatCodingScoreLabel(extractCodingScore(entry)) : "unavailable";
  return { label, color: colorForCodingScore(label) };
}
  ...
  if (parsed.command === "provider:list") {
    ...
    const providersWithCredit = await Promise.all(
      effectiveProviders.providers.map(async (provider) => ({
        ...provider,
        coding_info: await fetchProviderCodingInfo(provider, fetchFn, env, codingCache),
