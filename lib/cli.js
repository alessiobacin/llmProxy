...
const COMMAND_HELP = {
  ...
  "provider:add": {
    usage: "llmproxy provider:add <id> [--name <name>] [--api-key <key>] [--model <model>] [--plan <plan>]",
    description: "Aggiunge un provider noto: Copilot via device flow, gli altri provider tramite API key e modello predefinito.",
    ...
    example: "llmproxy provider:add qwen --api-key sk-sp-... --model qwen3.7-max --plan subscription",
  },
  "provider:key": {
    usage: "llmproxy provider:key <id> --api-key <key> [--model <model>] [--plan <plan>]",
    description: "Aggiorna o imposta la API key di un provider gia' configurato con autenticazione api_key.",
    when: "Usalo quando devi ruotare una chiave esistente senza ricreare da zero il provider.",
    example: "llmproxy provider:key qwen --api-key sk-sp-... --plan subscription",
  },
  "provider:available": {
...
function formatProviderList(providers) {
  return providers.map((provider, index) => `${index + 1}. ${provider.id} (${provider.name})${formatProviderPlanSuffix(provider)}`).join("\n");
}
...
function formatProviderStatus(providers) {
  ...
  providers.forEach((provider, index) => {
    ...
    lines.push(`${index + 1}. ${provider.id} (${provider.name})${activeSuffix} provider=${provider.provider || "copilot"} auth=${provider.auth_type || "oauth"} state=${state}${modelSuffix}${formatProviderPlanSuffix(provider)}`);
  });
  return lines.join("\n");
}
...
async function runCli(argv, options = {}) {
    ...
    for (const provider of selectedTargets) {
      ...
      try {
        const response = await runLocalProxyTestRequest(
          fetchFn,
          `${getProxyBaseUrl({ env, dataRoot: paths.dataRoot })}/v1/messages`,
          {
            method: "POST",
            headers: {
  ...
  if (parsed.command === "provider:add") {
    ...
    if (!providerId) {
      stderr.write("Provider id richiesto. Uso: llmproxy provider:add <id> [--name <name>] [--api-key <key>] [--model <model>] [--plan <plan>]\n");
      return 1;
    }
    ...
    if (providerInfo.auth === "api_key") {
      ...
      stdout.write(`Provider configurato con API key: ${providerId} (default model: ${defaultModel}${planSuffix}).\n`);
      return 0;
    }
  ...
  if (parsed.command === "provider:key") {
    ...
    if (!providerId || !apiKey) {
      stderr.write("Uso: llmproxy provider:key <id> --api-key <key> [--model <model>] [--plan <plan>]\n");
      return 1;
    }
    ...
    const planSuffix = providerId === "qwen" ? `, plan: ${formatQwenPlanLabel(endpointVariant)}` : "";
    stdout.write(`API key aggiornata per provider ${providerId} (default model: ${nextDefaultModel}${planSuffix}).\n`);
    return 0;
  }
