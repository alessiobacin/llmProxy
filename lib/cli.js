...
const COMMAND_HELP = {
  ...
  "provider:test": {
    usage: "llmproxy provider:test [--all-proxies]",
    description: "Testa la capacita' di visione di tutti i provider configurati. Con --all-proxies testa ogni combinazione provider × proxy.",
  ...
  "test": {
    usage: "llmproxy test [-i|--inference] [--all-providers] [<id>|<index>]",
    description: "Esegue un probe rapido dei provider oppure, con -i, una vera inferenza contro il proxy locale. Con -i si puo' specificare un ID provider (es. opencode-alessio) o un indice numerico (es. 1) per testare un singolo provider invece della fallback chain.",
  ...
  if (parsed.command === "test") {
    ...
    if (inferenceMode) {
      ...
      try {
        ...
        stdout.write(`inference: ok${finalProvider || finalModel ? ` (${[finalProvider, finalModel].filter(Boolean).join(" | ")})` : ""} [${elapsedMs}ms]\n`);
        if (allProviders) {
