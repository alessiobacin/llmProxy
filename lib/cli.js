...
async function readRemotePackageVersion(fetchFn, repo = "alessiobacin/llmProxy") {
  if (typeof fetchFn !== "function") return "";
  const response = await fetchFn(`https://raw.githubusercontent.com/${repo}/main/package.json`, {
    method: "GET",
...
function runSelfUpdate(commandRunner = spawnSync, repo = "alessiobacin/llmProxy", locale = "it") {
  const script = [
    ...
    '  fi',
    'done',
    'if [ -z "$new_bin" ] && [ -x "$npm_prefix/bin/llmproxy" ]; then',
    ...
    'fi',
    'if [ "$version_output" != "$target_version" ] && [ -f "$package_cli" ]; then',
...
async function runCli(argv, options = {}) {
    ...
    for (const provider of selectedTargets) {
      ...
      try {
        ...
        const assistantText = Array.isArray(payload?.content)
          ...
          : "";
        const visibleAssistantText = sanitizeTestAssistantText(assistantText);
        let summary = readLatestRequestSummary(paths.logsDir, providerStartedAtIso);
        if (!visibleAssistantText && !(summary?.success && String(summary?.finalProvider || "").trim() === provider.id)) {
          summary = await waitForLatestRequestSummary(paths.logsDir, {
        ...
        stdout.write(`${provider.id}: ok (${(finalModel || model)})\n`);
      } catch (error) {
        const errorMessage = error.message || String(error || "");
  ...
  if (parsed.command === "update") {
    ...
    try {
      const remoteVersion = await readRemotePackageVersion(fetchFn);
