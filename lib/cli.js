...
function runSelfUpdate(commandRunner = spawnSync, repo = "alessiobacin/llmProxy", locale = "it") {
  const script = [
    ...
    '  if [ -n "$installed_bin" ] && [ "$installed_bin" != "$new_bin" ] && [ "$installed_bin" != "$current_bin" ] && [ "$installed_bin" != "$global_bin_path" ]; then',
      ...
      try {
        ...
        if (!probe.ok) {
          ...
          if (probe.kind === "missing-model") {
            ...
            stdout.write(`${provider.id}: fail HTTP ${probe.status} (${model})\n`);
          } else {
