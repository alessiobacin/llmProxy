...
function isDockerServiceRunning(commandRunner, composeFile, serviceName) {
  const result = runDockerCompose(commandRunner, composeFile, ["ps", "--status", "running", "--services", serviceName]);
...
function ensureDockerServiceRunning({ commandRunner, env, paths, targetPlatform }) {
  ...
  const upResult = runDockerCompose(commandRunner, composeFile, ["up", "-d", "--build", serviceName], { invocation: dockerCompose });
  ...
  if (parsed.command === "status") {
    ...
    if (parsed.flags.docker) {
      ...
      if (!dockerCompose.dockerAvailable) {
        ...
        const dockerResult = runDockerCompose(spawnSync, composeFile, ["ps", "--format", "table"], { invocation: dockerCompose });
