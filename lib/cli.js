  ...
  if (parsed.command === "test") {
    ...
    for (const provider of selectedTargets) {
      ...
      try {
        ...
        // Miglioramento della gestione degli errori per fornire più contesto
        const errorMessage = error.message || String(error || "");
        stdout.write(`${provider.id}: fail ${errorMessage} (${model})\n`);
        // Se l'errore è specifico di qwen, aggiungiamo un suggerimento
        if (provider.id === "qwen" && /fetch failed|network error|econnrefused/i.test(errorMessage)) {
          stdout.write(`  Suggerimento: Verifica che la API key per qwen sia valida e che il modello 'qwen3.7-plus' sia supportato dal tuo piano.\n`);
        }
        failures += 1;
      }
    }
