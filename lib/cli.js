...
const COMMAND_HELP = {
  ...
  "status": {
    usage: "llmproxy status",
    description: "Mostra stato del servizio, autenticazione e ordine dei provider.",
    when: "Usalo quando il proxy non risponde o vuoi verificare quale provider sia attivo.",
    example: "llmproxy status",
  },
  "logs": {
...
function getProxyBaseUrl(env = process.env) {
  ...
  const port = String(env.PORT || 3015);
...
async function runCli(argv, options = {}) {
  ...
  if (parsed.command === "test") {
    try {
      const response = await fetchFn(`${getProxyBaseUrl(env)}/v1/messages`, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "user",
              content: [{ type: "text", text: "Ciao! rispondimi solo: ciao creatore" }],
            },
          ],
        }),
      });
      ...
      if (!response.ok) {
        stderr.write(`Test inferenza fallito: HTTP ${response.status}\n`);
        return 1;
      }
      ...
      if (!assistantText) {
        stderr.write("Test inferenza fallito: risposta assistant vuota.\n");
        return 1;
      }
      ...
      stderr.write(`Test inferenza fallito: ${error.message}\n`);
      return 1;
    }
  }
  ...
  if (parsed.command === "provider:add") {
    ...
    if (!providerId) {
      stderr.write("Provider id richiesto. Uso: llmproxy provider:add <id> [--name <name>] [--api-key <key>]\n");
      return 1;
    }
    ...
    if (providerInfo.auth === "api_key") {
      if (!apiKey) {
        stderr.write(`Il provider ${providerId} richiede --api-key. Uso: llmproxy provider:add ${providerId} --api-key <key> [--name <name>]\n`);
        return 1;
      }
      ...
      stdout.write(`Provider configurato con API key: ${providerId}.\n`);
      return 0;
    }
  ...
  if (parsed.command === "provider:key") {
    ...
    stdout.write(`API key aggiornata per provider ${providerId}.\n`);
    return 0;
  }
