...
const COMMAND_HELP = {
  ...
  "config:get": {
    ...
    example: "llmproxy config:get LLMPROXY_PRICE_PERFORMANCE_ROUTING --scope global",
  },
  "config:set": {
    ...
    example: "llmproxy config:set LLMPROXY_PRICE_PERFORMANCE_ROUTING 1 --scope global",
  },
  "config:unset": {
...
function formatProviderList(providers, options = {}) {
  ...
  return providers.map((provider, index) => {
    ...
    const proxyValue = provider.proxy_url
      ...
      : null;
    const details = [
      `   model=${modelValue} coding=${codingValue}${formatVisionSuffix(provider, useColor)}${formatProviderPlanSuffix(provider, useColor)}`,
      `   credit=${creditValue}`,
      `   price=${currentPrice}`,
      `   best=${bestProvider} (${bestPrice})`,
      proxyValue ? `   proxy=${proxyValue}` : null,
      `   bench=${provider.benchmark_info?.ok ? `${provider.benchmark_info.durationMs} ms` : "n/a"}`,
    ].filter(Boolean);
...
async function runCli(argv, options = {}) {
  ...
  if (parsed.command === "provider:list") {
    ...
    const benchmarkCache = providerStore.__llmproxyProviderBenchmark
      ? providerStore.__llmproxyProviderBenchmark
      : (() => {
          const bmPath = path.join(paths.dataRoot, DEFAULT_BENCHMARK_FILE);
          const store = readBenchmarkStore(bmPath);
          return { getLastResult: (pid) => store.providers[pid] || null };
        })();
    const providersWithCredit = await Promise.all(
      effectiveProviders.providers.map(async (provider) => ({
        ...
        benchmark_info: benchmarkCache.getLastResult(provider.id),
      })),
    );
