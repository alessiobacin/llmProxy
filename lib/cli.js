...
const { parse: parseJsonc, modify: modifyJsonc, applyEdits: applyJsoncEdits } = require("jsonc-parser");

// --- Spinner minimale (solo TTY) per segnalare operazioni in corso ---
function startSpinner(message) {
  const frames = ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"];
  let i = 0;
  process.stdout.write(`\x1b[36m${frames[0]} ${message}\x1b[0m`);
...
const { probeApiKeyProviderModel, normalizeQwenEndpointVariant, parseProviderModelPreferences, makeProxyFetch } = require("./copilot-proxy");
const {
  loadRuntimeEnv,
...
const { createProviderReordering, readReorderingStore, buildDefaultProbeFn, fetchSpeedMetric, fetchBenchmarkMetric, DEFAULT_REORDERING_FILE } = require("./provider-reordering");
const {
...
function buildProxyAgentUrl(proxyUrlRaw, proxyApiKey) {
  ...
  const normalizedRaw = String(proxyUrlRaw || "").trim();
  const withScheme = normalizedRaw && !/^[a-z][a-z0-9+.-]*:\/\//i.test(normalizedRaw)
    ? `http://${normalizedRaw}`
    : normalizedRaw;
  let parsed = null;
  try {
    parsed = withScheme ? new URL(withScheme) : null;
  } catch {
    parsed = null;
  }
  ...
  if (parsed?.hostname) {
    ...
    let auth = "";
    if (key) {
      const user = parsed.username || "proxy";
      auth = `${encodeURIComponent(user)}:${encodeURIComponent(key)}@`;
    } else if (parsed.username) {
      const pw = parsed.password ? encodeURIComponent(parsed.password) : "";
      auth = `${encodeURIComponent(parsed.username)}:${pw}@`;
    }
    const host = parsed.hostname.includes(":") ? `[${parsed.hostname}]` : parsed.hostname;
    const port = parsed.port || "7064";
...
const AVAILABLE_PROVIDER_SPECS = [
  ...
  { id: "vercel-ai-gateway", auth: "api_key", displayName: "Vercel AI Gateway", aliases: ["ai-gateway", "vercel"] },
  { id: "meta", auth: "api_key", displayName: "Meta AI", aliases: [] },
];
...
const SHORT_COMMAND_ALIASES = {
  ...
  "pi:s": "pi:setup",
  // Gruppo vscode
  "vsc:cs": "vscode-chat:setup",
  "vsc:cc": "vscode-claude:setup",
  // Client OpenAI-compatible a catalogo statico
  "oc:s": "opencode:setup",
  "con:s": "continue:setup",
  "zed:s": "zed:setup",
  // Gruppo stats
  "sa:r": "stats:reset",
  // Gruppo install
...
function resolveApiKeyProviderKind(rawInstanceId, rawProviderFlag) {
  const explicitProviderKind = normalizeKnownProviderId(rawProviderFlag);
  if (explicitProviderKind && KNOWN_PROVIDERS[explicitProviderKind]?.auth === "api_key") {
    return explicitProviderKind;
  }

  const normalizedInstanceId = normalizeKnownProviderId(rawInstanceId);
  if (normalizedInstanceId && KNOWN_PROVIDERS[normalizedInstanceId]?.auth === "api_key") {
    return normalizedInstanceId;
  }

  const knownApiKeyIds = AVAILABLE_PROVIDER_SPECS
    .filter((provider) => provider.auth === "api_key")
    .flatMap((provider) => [provider.id, ...(provider.aliases || [])])
    .map((providerId) => normalizeKnownProviderId(providerId))
    .filter(Boolean)
    .sort((left, right) => right.length - left.length);

  for (const providerId of knownApiKeyIds) {
    if (normalizedInstanceId === providerId || normalizedInstanceId.startsWith(`${providerId}-`)) {
      return providerId;
    }
  }
...
const COMMAND_HELP = {
  ...
  "provider:test": {
    usage: "llmproxy provider:test [--vision] [--all-proxies] [--proxy <url>] [--provider <id>]",
    description: "Testa i provider configurati con inferenza testuale. Usa --vision per testare la visione immagini; con --all-proxies o --proxy esegue inferenza su ogni combinazione provider × proxy.",
    when: "Usalo dopo aver configurato i provider per verificare che funzionino con un test di inferenza completo, oppure con --vision per testare la visione immagini.",
    example: "llmproxy provider:test",
    englishExample: "llmproxy provider:test",
  },
  "provider:status": {
  ...
  "provider:update": {
    usage: "llmproxy provider:update <id> [--vision <true|false>] [--free-model <true|false>] [--name <name>]",
    description: "Aggiorna metadati di un provider gia' configurato senza toccare le credenziali o il modello. --vision abilita/disabilita la partecipazione al fallback delle richieste con immagini. --free-model attiva/disattiva il blocco dell'auto-escalation per quella coppia provider/modello. --name cambia il nome visualizzato.",
    when: "Usalo quando vuoi correggere velocemente il flag vision dopo un test, marcare un provider come gratuito gratuito, o rinominare un'istanza per distinguerla da altre dello stesso tipo.",
    example: "llmproxy provider:update opencode-bacin --vision false --free-model true --name 'OpenCode (Bacin)'",
  },
  "provider:rename": {
  ...
  "stats:reset": {
    usage: "llmproxy stats:reset",
    description: "Azzera tutte le statistiche di utilizzo (metering records).",
  ...
  "config:list": {
    usage: "llmproxy config:list [--project|--service|--scope <project|global|service>]",
    ...
    example: "llmproxy config:list --scope global",
  },
  "config:get": {
    usage: "llmproxy config:get <key> [--project|--service|--scope <project|global|service>]",
    ...
    example: "llmproxy config:get LLMPROXY_REORDERING --scope service",
  },
  "config:set": {
    usage: "llmproxy config:set <key> <value> [--project|--service|--scope <project|global|service>]",
    ...
    example: "llmproxy config:set LLMPROXY_REORDERING price-speed-power --scope service",
  },
  "config:unset": {
    usage: "llmproxy config:unset <key> [--project|--service|--scope <project|global|service>]",
    ...
    example: "llmproxy config:unset ANTHROPIC_DEFAULT_MODEL --scope global",
  },
  "test": {
    usage: "llmproxy test [-i|--inference] [--all-providers] [--proxy <url>] [--provider <id>] [<id>|<index>]",
    description: "Esegue un probe rapido dei provider oppure, con -i, una vera inferenza contro il proxy locale. Con -i si puo' specificare un ID provider (es. opencode-alessio) o un indice numerico (es. 1) per testare un singolo provider invece della fallback chain. Con --proxy <url> si forza un proxy specifico.",
  ...
  "pi:setup": {
    usage: "llmproxy pi:setup",
    description: "Scrive .pi/models.json e .pi/settings.json per usare llmproxy come provider in PI Agent.",
    when: "Usalo nel progetto che vuoi usare con PI Agent, dopo aver configurato llmproxy con almeno un provider.",
    example: "llmproxy pi:setup",
  },
  "vscode-chat:setup": {
    usage: "llmproxy vscode-chat:setup",
    description: "Configura VS Code Chat con llmproxy come provider OpenAI, includendo tutti i modelli disponibili.",
    when: "Usalo per abilitare llmproxy come backend nella Chat di VS Code (Copilot Chat).",
    example: "llmproxy vscode-chat:setup",
  },
  "opencode:setup": {
    usage: "llmproxy opencode:setup",
    description: "Configura il provider OpenAI-compatible llmProxy nel file globale OpenCode, con Auto e tutti i modelli espliciti.",
    when: "Usalo se scegli i modelli in OpenCode tramite la configurazione statica ~/.config/opencode/opencode.json(c).",
    example: "llmproxy opencode:setup",
  },
  "continue:setup": {
    usage: "llmproxy continue:setup",
    description: "Aggiunge Auto e tutti i modelli llmProxy al config.yaml globale di Continue.",
    when: "Usalo per rendere llmProxy selezionabile nel picker statico dell'estensione Continue.",
    example: "llmproxy continue:setup",
  },
  "zed:setup": {
    usage: "llmproxy zed:setup",
    description: "Configura il provider OpenAI-compatible llmProxy nei settings globali di Zed, con catalogo statico di modelli.",
    when: "Usalo per aggiungere llmProxy al picker Agent di Zed; la chiave resta nel keychain Zed o nella variabile LLMPROXY_API_KEY.",
    example: "llmproxy zed:setup",
  },
  "vscode-claude:setup": {
    usage: "llmproxy vscode-claude:setup [--model <indice>]",
    description: "Configura l'estensione Claude Code in VS Code con llmproxy come backend e tutti i modelli disponibili.",
    when: "Usalo per collegare l'estensione Claude Code in VS Code al proxy locale llmproxy.",
    example: "llmproxy vscode-claude:setup --model 2",
  },
  "proxy:add": {
    usage: "llmproxy proxy:add <url> [--name <nome>]",
    description: "Aggiunge un proxy alla registry (id = dominio/hostname dell'URL). Il formato URL include credenziali: http://user:pass@host:port. --name e' solo un'etichetta leggibile, NON e' l'id: per rimuovere o riordinare usa il dominio. Riaggiungere lo stesso dominio sovrascrive l'URL esistente (upsert voluto).",
    when: "Usalo per registrare un proxy da usare in rotazione con --proxy (senza valore).",
    example: "llmproxy proxy:add http://user:key@dc.decodo.com:10001 --name 'Proxy primario'",
  },
  "proxy:list": {
  ...
  "proxy:reorder": {
    usage: "llmproxy proxy:reorder <id1> <id2> ...",
    description: "Riordina i proxy specificando gli ID nel nuovo ordine di failover.",
    when: "Usalo per cambiare l'ordine di failover dei proxy.",
    example: "llmproxy proxy:reorder proxy-a.com proxy-b.com",
  },
  "proxy:test": {
...
Esecuzione proxy:
  ...
  llmproxy stats:reset                      ${s("stats:reset")}  azzera le statistiche di utilizzo
...
Provider:
  ...
  llmproxy provider:test [--vision] [--all-proxies] ${s("provider:test")}  inferenza testuale; con --vision testa la visione, con --all-proxies testa ogni proxy
  ...
  llmproxy provider:reorder                   ${s("provider:reorder")}  riordina i provider in base ai criteri LLMPROXY_REORDERING
  llmproxy provider:update <id> ...        ${s("provider:update")}  aggiorna vision, free-model o name di un provider
...
Proxy (registro e rotazione):
  llmproxy proxy:add <url> [--name]        ${s("proxy:add")}  aggiunge un proxy alla registry
  llmproxy proxy:list                      ${s("proxy:list")}  elenca i proxy registrati in ordine di failover
  llmproxy proxy:remove <id>               ${s("proxy:remove")}  rimuove un proxy per ID (dominio)
  llmproxy proxy:reorder <id1> <id2> ...   ${s("proxy:reorder")}  riordina i proxy (primo = prioritario)
  llmproxy proxy:test                      ${s("proxy:test")}  testa la connettivita' di tutti i proxy
  llmproxy provider:test --all-proxies     ${s("provider:test")}  esegue un test reale di inferenza provider × proxy
  (usa --proxy senza valore per rotazione automatica al posto di --proxy <url>)
...
Modelli e Claude Code:
  ...
  llmproxy config:list [--scope ...]          ${s("config:list")}  mostra le variabili configurabili gestite da llmproxy
  ...
  llmproxy pi:setup                       ${s("pi:setup")}  scrive .pi/models.json e settings.json per PI Agent
  llmproxy vscode-chat:setup              ${s("vscode-chat:setup")}  configura VS Code Chat con llmproxy (tutti i modelli)
  llmproxy opencode:setup                 ${s("opencode:setup")}  configura OpenCode con Auto e catalogo statico llmProxy
  llmproxy continue:setup                 ${s("continue:setup")}  configura Continue con Auto e catalogo statico llmProxy
  llmproxy zed:setup                      ${s("zed:setup")}  configura Zed con Auto e catalogo statico llmProxy
  llmproxy vscode-claude:setup [--model <indice>] ${s("vscode-claude:setup")}  configura Claude Code in VS Code con llmproxy
...
Quando usare cosa:
  ...
  4. Proxy rotazione: proxy:add <url> -> proxy:list -> provider:add ... --proxy ...
  5. Aggiornamento CLI: update
  6. Rimozione completa: uninstall
...
function compareReleaseVersions(left, right) {
  ...
  if (leftParts.some((part) => !Number.isFinite(part)) || rightParts.some((part) => !Number.isFinite(part))) {
    return null;
  }
  const maxLength = Math.max(leftParts.length, rightParts.length);
  for (let index = 0; index < maxLength; index += 1) {
    const current = leftParts[index] ?? 0;
    const remote = rightParts[index] ?? 0;
    if (current > remote) return 1;
    if (current < remote) return -1;
  }
  return 0;
}
...
function readEmbeddedReleaseNotes(packageRoot, version, locale = "it") {
  try {
    const notesFile = path.join(packageRoot, "release-notes.json");
    const notes = JSON.parse(fs.readFileSync(notesFile, "utf8"));
    const versionNotes = notes[String(version || "").trim()];
    if (!versionNotes) return [];
    const normalizedLocale = normalizeLocale(locale);
    return Array.isArray(versionNotes[normalizedLocale]) ? versionNotes[normalizedLocale].filter((note) => String(note || "").trim()) : [];
  } catch {
    return [];
  }
}
...
function getPlatformCommandCandidates(command, platform = process.platform) {
  const normalized = String(command || "").trim();
  if (!normalized) return [];
  if (platform !== "win32") return [normalized];
  if (normalized.includes(".") || normalized.includes("\\") || normalized.includes("/")) return [normalized];
  if (normalized === "npm" || normalized === "pnpm" || normalized === "npx") {
    return [`${normalized}.cmd`, normalized];
  }
  return [normalized];
}

function runPlatformCommand(commandRunner, command, args = [], platform = process.platform) {
  let lastResult = null;
  for (const candidate of getPlatformCommandCandidates(command, platform)) {
    const result = platform === "win32" && candidate.toLowerCase().endsWith(".cmd")
      ? runCommand(commandRunner, "cmd.exe", ["/d", "/s", "/c", [quoteWindowsCmdArg(candidate), ...args.map((arg) => quoteWindowsCmdArg(arg))].join(" ")])
      : runCommand(commandRunner, candidate, args);
    lastResult = result;
    if (result && result.status === 0) return result;
    const errorCode = result?.error?.code || result?.code || "";
    const stderrText = String(result?.stderr || "");
    const missingCommand = errorCode === "ENOENT" || /not recognized|cannot find/i.test(stderrText);
    if (!missingCommand) return result;
  }
  return lastResult || { status: 1, stdout: "", stderr: "" };
}

function commandIsAvailableForPlatform(commandRunner, command, platform = process.platform, args = ["--version"]) {
  const result = runPlatformCommand(commandRunner, command, args, platform);
  return result && result.status === 0;
}
...
function canUseSudoNonInteractively(commandRunner) {
  if (!commandIsAvailable(commandRunner, "sudo")) return false;
  const result = runCommand(commandRunner, "sudo", ["-n", "true"]);
  return !!result && result.status === 0;
}

function isWindowsSessionElevated(commandRunner = spawnSync) {
  const result = runCommand(commandRunner, "powershell.exe", [
    "-NoProfile",
    "-Command",
...
function getWindowsServiceState(commandRunner = spawnSync, label = "llmproxy") {
  const result = runCommand(commandRunner, "sc.exe", ["query", label]);
  const stdout = String(result?.stdout || "");
  const stderr = String(result?.stderr || "");
  const combined = `${stdout}\n${stderr}`;
  const normalized = combined.toLowerCase();
  const missing = /does not exist|1060/i.test(normalized);
  const active = normalized.includes("running") || /\b(stato|state)\b[\s:]+4\b/i.test(combined);
  return {
...
function getPrerequisiteFixCommands(key, options = {}) {
  ...
  if (platform === "win32") {
    ...
    if (key === "windows-service-admin") return locale === "en"
      ? ["Run PowerShell as Administrator, then retry: llmp up", "Or stop the service as Administrator first: llmp service:stop"]
      : ["Esegui PowerShell come Amministratore, poi riprova: llmp up", "Oppure arresta prima il servizio come Amministratore: llmp service:stop"];
    return [];
  }
...
function resolveUpdatePrerequisites(options = {}) {
  ...
  const runtimeEntry = resolveServiceEntryFile({ env, packageRoot, targetPlatform });
  ...
  if (!commandIsAvailableForPlatform(commandRunner, "git", targetPlatform)) {
  ...
  // }
  const npmAvailable = commandIsAvailableForPlatform(commandRunner, "npm", targetPlatform);
  if (!npmAvailable) {
    addMissing("npm", "npm non trovato nel PATH.", "npm is not available in PATH.");
  }

  if (npmAvailable) {
    const npmPrefixResult = runPlatformCommand(commandRunner, "npm", ["prefix", "-g"], targetPlatform);
    ...
    if (!npmPrefixResult || npmPrefixResult.status !== 0 || !npmPrefix) {
    ...
    } else if (!canWritePath(npmPrefix)) {
      if (!commandIsAvailableForPlatform(commandRunner, "sudo", targetPlatform)) {
        addMissing(
          "npm-write",
      ...
      } else if (!canUseSudoNonInteractively(commandRunner)) {
        addMissing(
          "sudo-ticket",
          `Il prefisso globale npm non e' scrivibile (${npmPrefix}) e questa CLI non puo' richiedere la password sudo in modo interattivo. Esegui prima \`sudo -v\` oppure rilancia \`sudo llmproxy update\`.`,
          `The global npm prefix is not writable (${npmPrefix}) and this CLI cannot prompt for a sudo password interactively. Run \`sudo -v\` first or rerun \`sudo llmproxy update\`.`,
        );
      }
    }
  }
  ...
  if (targetPlatform === "win32") {
    const serviceState = getWindowsServiceState(commandRunner, "llmproxy");
    if (serviceState.active && !isWindowsSessionElevated(commandRunner)) {
      addMissing(
        "windows-service-admin",
        "Il servizio Windows `llmproxy` e' attivo e questa sessione PowerShell non e' elevata. Esegui `llmp up` da una PowerShell avviata come Amministratore, oppure arresta prima il servizio con privilegi amministrativi.",
        "The Windows `llmproxy` service is active and this PowerShell session is not elevated. Run `llmp up` from an Administrator PowerShell, or stop the service first with administrative privileges.",
      );
    }
  }
  ...
  if (requiresDockerRuntime) {
    ...
    const dockerAvailable = commandIsAvailableForPlatform(commandRunner, "docker", targetPlatform);
    if (!dockerAvailable) {
      ...
      if (!commandIsAvailableForPlatform(commandRunner, "docker", targetPlatform, ["info"])) {
        addMissing(
          "docker-daemon",
...
function resolvePersistentInstallPrerequisites(options = {}) {
  ...
  if (targetPlatform === "win32") {
    if (!commandIsAvailableForPlatform(commandRunner, "powershell.exe", targetPlatform, ["-NoProfile", "-Command", "Get-Command", "sc.exe"])) {
      addMissing(
        "sc",
  ...
  if (targetPlatform === "linux" && !commandIsAvailableForPlatform(commandRunner, "systemctl", targetPlatform, ["--version"])) {
    addMissing(
      "systemctl",
  ...
  if (npmAvailable) {
    ...
    if (!npmPrefixResult || npmPrefixResult.status !== 0 || !npmPrefix) {
      ...
      if (!commandIsAvailableForPlatform(commandRunner, "sudo", targetPlatform)) {
        ...
        addMissing(
          "sudo-ticket",
          `Il prefisso globale npm non e' scrivibile (${npmPrefix}) e questa CLI non puo' richiedere la password sudo in modo interattivo. Esegui prima \`sudo -v\` oppure rilancia il comando con \`sudo\`.`,
          `The global npm prefix is not writable (${npmPrefix}) and this CLI cannot prompt for a sudo password interactively. Run \`sudo -v\` first or rerun the command with \`sudo\`.`,
        );
      }
    }
  }
  ...
  if (requiresDockerRuntime) {
    ...
    if (dockerCompose.dockerAvailable && !commandIsAvailableForPlatform(commandRunner, "docker", targetPlatform, ["info"])) {
      addMissing(
        "docker-daemon",
        "Il daemon Docker non e' raggiungibile per l'utente corrente (`docker info` fallisce).",
        "The Docker daemon is not reachable for the current user (`docker info` failed).",
...
function formatProviderList(providers, options = {}) {
  ...
  return providers.map((provider, index) => {
    ...
    if (provider.proxy_rotation) {
      proxyValue = colorize("proxy:rotation", "magenta", useColor);
    } else if (provider.proxy_url) {
      proxyValue = colorize(`proxy:${provider.proxy_url}`, "magenta", useColor);
    } else {
      proxyValue = colorize("proxy:no", "dim", useColor);
    }
    const benchSpeed = provider.bench_metric || "n/a";
    const visionSuffix = formatVisionSuffix(provider, useColor);
    const freeModelSuffix = formatFreeModelSuffix(provider, useColor);
    const planSuffix = formatProviderPlanSuffix(provider, useColor);
    const visibilityLine = [visionSuffix.trim(), freeModelSuffix.trim()].filter(Boolean).join(" ");
    const details = [
      `   model=${modelValue}`,
      `   credit=${creditValue}`,
      `   coding=${codingValue}${planSuffix}`,
      visibilityLine ? `   ${visibilityLine}` : null,
      `   price=${currentPrice}`,
      `   best=${bestProvider} (${bestPrice})`,
      proxyValue ? `   proxy=${proxyValue}` : null,
      `   bench=${benchSpeed}`,
    ].filter(Boolean);

    return `${linePrefix} ${providerId} (${providerName})\n${details.join("\n")}`;
  }).join("\n\n");
}
...
async function fetchProviderCodingInfo(provider, fetchFn, cache = new Map()) {
  // Prefer stored coding_score (backfilled at provider insertion, no need to fetch live every time)
  if (typeof provider?.coding_score === "number" && Number.isFinite(provider.coding_score)) {
    const label = formatCodingScoreLabel(provider.coding_score);
...
function formatProviderStatus(providers) {
  ...
  providers.forEach((provider, index) => {
    ...
    const visionSuffix = formatVisionSuffix(provider);
    const planSuffix = formatProviderPlanSuffix(provider);
    const visionLine = visionSuffix ? `   ${visionSuffix.trim()}` : "";
    lines.push(`${index + 1}. ${provider.id} (${provider.name})${activeSuffix} provider=${provider.provider || "copilot"} auth=${provider.auth_type || "oauth"} state=${state}${modelSuffix}${planSuffix}${visionLine ? "\n" + visionLine : ""}`);
  });
  return lines.join("\n");
}
...
function formatBreakdownSection(title, entriesObject = {}, useColor = false) {
  const entries = sortBreakdownEntries(entriesObject);
  if (entries.length === 0) {
    return {
      header: title,
      rows: [],
      columns: ["Name", "Req", "Total", "In", "Out"],
    };
  }
  const rows = entries.map(([key, value]) => {
    ...
    return { name: key, requests, total, input, output };
  });
  return { header: title, rows, columns: ["Name", "Req", "Total", "In", "Out"] };
}
...
function renderStatsTable(stats = {}, source = "unknown", useColor = false) {
  ...
  const providers = formatBreakdownSection("Providers", stats.by_provider || {}, useColor);
  const models = formatBreakdownSection("Models", stats.by_model || {}, useColor);

  const sections = [
    { label: "Source", value: source },
    { label: "Requests", value: `${formatNumber(totalRequests)} | Success: ${formatNumber(successCount)} | Errors: ${formatNumber(errorCount)}` },
    { label: "Tokens", value: `total=${formatNumber(total)} in=${formatNumber(input)} out=${formatNumber(output)}` },
  ];
  if (stats.earliest_timestamp || stats.latest_timestamp) {
    sections.push({ label: "Range", value: `${stats.earliest_timestamp || "n/a"} → ${stats.latest_timestamp || "n/a"}` });
  }

  const allRows = [
    ...providers.rows.map((r) => ({ ...r, section: "Providers" })),
    ...models.rows.map((r) => ({ ...r, section: "Models" })),
  ];

  const nameWidth = Math.max(
    5,
    ...allRows.map((r) => r.name.length),
    ...["Name", "Provider", "Model"].map((s) => s.length),
  );
  const numCols = ["requests", "total", "input", "output"];
  const colWidths = {};
  for (const col of numCols) {
    colWidths[col] = Math.max(
      5,
      ...allRows.map((r) => formatNumber(r[col]).length),
    );
  }
  ...
  const innerWidth = nameWidth + colWidths.requests + colWidths.total + colWidths.input + colWidths.output + 20;
  const width = Math.max(innerWidth, 50);

  function horizontalBar(left, mid, right, fill = border) {
    return left + fill.repeat(width - 2) + right;
  }

  function sectionDivider(label) {
    const labelText = `─ ${label} `;
    const remaining = width - 2 - labelText.length;
    return midLeft + labelText + border.repeat(Math.max(0, remaining)) + midRight;
  }

  function tableHeader() {
    const nameCol = padRight("Name", nameWidth);
    const reqCol = padLeft("Req", colWidths.requests);
    const totalCol = padLeft("Total", colWidths.total);
    const inCol = padLeft("In", colWidths.input);
    const outCol = padLeft("Out", colWidths.output);
    const row = `${pipe} #  ${nameCol}  ${reqCol}  ${totalCol}  ${inCol}  ${outCol} ${pipe}`;
    if (useColor) {
      return colorize(row, "bright", true);
    }
    return row;
  }

  function tableRow(index, row) {
    const nameCol = padRight(row.name, nameWidth);
    const reqCol = padLeft(formatNumber(row.requests), colWidths.requests);
    const totalCol = padLeft(formatNumber(row.total), colWidths.total);
    const inCol = padLeft(formatNumber(row.input), colWidths.input);
    const outCol = padLeft(formatNumber(row.output), colWidths.output);
    const line = `${pipe} ${index}  ${nameCol}  ${reqCol}  ${totalCol}  ${inCol}  ${outCol} ${pipe}`;
    if (useColor) {
  ...
  const lines = [];
  lines.push(horizontalBar(topLeft, border, topRight));
  for (const section of sections) {
    const labelText = colorize(section.label, "bright", useColor);
    const valueText = useColor ? colorize(section.value, "dim", true) : section.value;
    lines.push(`${pipe} ${labelText}: ${valueText}`);
  }

  lines.push(sectionDivider("Providers"));
  if (providers.rows.length === 0) {
    lines.push(`${pipe}   ${colorize("none", "dim", useColor)}`);
  } else {
    lines.push(tableHeader());
    providers.rows.forEach((row, i) => {
      lines.push(tableRow(i + 1, row));
    });
  }

  lines.push(sectionDivider("Models"));
  if (models.rows.length === 0) {
    lines.push(`${pipe}   ${colorize("none", "dim", useColor)}`);
  } else {
    if (providers.rows.length === 0) lines.push(tableHeader());
    models.rows.forEach((row, i) => {
      lines.push(tableRow(i + 1, row));
    });
  }

  lines.push(horizontalBar(botLeft, border, botRight));
  return lines.join("\n");
}
...
function resolveEventBusUrlForEnv(env = {}) {
  ...
  const profile = resolveRuntimeProfile({ env });
  const expectedPort = profile === "production" ? "7048" : profile === "staging" ? "6048" : "5048";
  if (explicit) return resolveServiceUrlForProfile(explicit.replace(/\/$/, ""), expectedPort);
...
function formatStatsReport(stats = {}, source = "unknown", useColor = false) {
  return renderStatsTable(stats, source, useColor);
}
...
function requestSummaryMatchesProvider(entry, provider) {
  const finalProvider = String(entry?.finalProvider || "").trim().toLowerCase();
  if (!finalProvider) return false;

  const providerId = String(provider?.id || "").trim().toLowerCase();
  const providerKind = String(provider?.provider || "").trim().toLowerCase();
  const providerMatch = finalProvider === providerId || (providerKind && finalProvider === providerKind);
  if (!providerMatch) return false;

  // Evita un summary stale di un'altra richiesta risolta dallo stesso
  // finalProvider (es. fallback): il modello richiesto deve corrispondere
  // al modello che il probe stava realmente inviando.
  const entryRequestedModel = String(entry?.requestedModel || "").trim().toLowerCase();
  if (entryRequestedModel) {
    const providerModel = String(provider?.effective_model || provider?.default_model || "").trim().toLowerCase();
    if (providerModel && entryRequestedModel !== providerModel) return false;
  }
  return true;
}
...
function formatReleaseNotes(version, locale = "it", options = {}) {
  ...
  const notesFromPackage = readEmbeddedReleaseNotes(options.packageRoot, version, normalizedLocale);
...
function runSelfUpdate(commandRunner = spawnSync, repo = "alessiobacin/llmProxy", locale = "it") {
  const script = [
    ...
    '        sudo rm -rf "$target_path" >/dev/null 2>&1 || true',
    '      fi',
    '    }',
    '  fi',
    '}',
    'collect_install_target() {',
    '  candidate_dir="$1"',
    ...
    '  install_targets=$( {',
    '    collect_install_target "${original_npm_root:+$original_npm_root/llmproxy}"',
    '    collect_install_target "${current_bin_prefix:+$current_bin_prefix/lib/node_modules/llmproxy}"',
    '    for discovered_bin in $existing_bins; do',
    '      [ -n "$discovered_bin" ] || continue',
    '      discovered_prefix=$(cd "$(dirname "$discovered_bin")/.." 2>/dev/null && pwd || true)',
    '      collect_install_target "${discovered_prefix:+$discovered_prefix/lib/node_modules/llmproxy}"',
    '    done',
    '    current_npm_root=$(npm root -g 2>/dev/null || true)',
    '    collect_install_target "${current_npm_root:+$current_npm_root/llmproxy}"',
    '    if command -v sudo >/dev/null 2>&1; then',
    '      sudo_npm_root=$(sudo npm root -g 2>/dev/null || true)',
    '      collect_install_target "${sudo_npm_root:+$sudo_npm_root/llmproxy}"',
    '    fi',
    '    pnpm_root=$(pnpm root -g 2>/dev/null || true)',
    '    collect_install_target "${pnpm_root:+$pnpm_root/llmproxy}"',
    '    collect_install_target "/usr/lib/node_modules/llmproxy"',
    '    collect_install_target "/usr/local/lib/node_modules/llmproxy"',
    '    collect_install_target "/opt/homebrew/lib/node_modules/llmproxy"',
    '  } | awk \'NF && !seen[$0]++\' )',
    '  for install_target in $install_targets; do',
    '    backup_install_dir "$install_target"',
    '    safe_remove_path "$install_target"',
    '  done',
    '}',
    'purge_existing_wrappers() {',
    '  wrapper_targets=$(printf "%s\\n" "$current_bin" "$current_short_bin" "$original_global_bin_path" "$original_global_short_bin_path" "/usr/bin/llmproxy" "/usr/local/bin/llmproxy" "/opt/homebrew/bin/llmproxy" "/usr/bin/llmp" "/usr/local/bin/llmp" "/opt/homebrew/bin/llmp" $existing_bins | awk \'NF && !seen[$0]++\')',
    '  for wrapper_target in $wrapper_targets; do',
    '    safe_remove_path "$wrapper_target"',
    '  done',
    '}',
    ...
    '  fi',
    '}',
    'ensure_runtime_home_permissions() {',
    '  runtime_home="$1"',
    '  runtime_owner="$2"',
    '  [ -n "$runtime_home" ] || return 0',
    '  mkdir -p "$runtime_home/logs" >/dev/null 2>&1 || {',
    '    if command -v sudo >/dev/null 2>&1; then',
    '      sudo mkdir -p "$runtime_home/logs" >/dev/null 2>&1 || true',
    '    fi',
    '  }',
    '  touch "$runtime_home/logs/service.out.log" "$runtime_home/logs/service.err.log" "$runtime_home/logs/metering.jsonl" >/dev/null 2>&1 || {',
    '    if command -v sudo >/dev/null 2>&1; then',
    '      sudo touch "$runtime_home/logs/service.out.log" "$runtime_home/logs/service.err.log" "$runtime_home/logs/metering.jsonl" >/dev/null 2>&1 || true',
    '    fi',
    '  }',
    '  if [ -n "$runtime_owner" ] && command -v sudo >/dev/null 2>&1; then',
    '    sudo chown -R "$runtime_owner":"$runtime_owner" "$runtime_home" >/dev/null 2>&1 || true',
    '  fi',
    '  chmod -R u+rwX "$runtime_home" >/dev/null 2>&1 || true',
    '}',
    'cleanup_global_service_port "${PORT:-7045}"',
    'npm uninstall -g llmproxy >/dev/null 2>&1 || true',
    'if command -v sudo >/dev/null 2>&1; then',
    '  sudo npm uninstall -g llmproxy >/dev/null 2>&1 || true',
    'fi',
    ...
    'fi',
    'runtime_owner=""',
    'if [ "$used_sudo" -eq 1 ] && [ -n "${SUDO_USER:-}" ]; then',
    '  runtime_owner="$SUDO_USER"',
    'else',
    '  runtime_owner="$(whoami)"',
    'fi',
    'ensure_runtime_home_permissions "$LLMPROXY_HOME" "$runtime_owner"',
    'compose_cmd=""',
    'compose_args=""',
...
function runSelfUpdateWindows(commandRunner = spawnSync, repo = "alessiobacin/llmProxy", locale = "it") {
  const psScript = [
    ...
    '}',
    '',
    'function Invoke-QuietNative([string]$FilePath, [string[]]$ArgumentList) {',
    '  $stdoutFile = Join-Path $tmpdir ([System.IO.Path]::GetRandomFileName())',
    '  $stderrFile = Join-Path $tmpdir ([System.IO.Path]::GetRandomFileName())',
    '  try {',
    '    $resolvedFilePath = $FilePath',
    '    $resolvedArguments = @($ArgumentList)',
    '    $extension = [System.IO.Path]::GetExtension($FilePath).ToLowerInvariant()',
    '    if ($FilePath -ieq "npm" -or $FilePath -ieq "npm.cmd" -or $extension -eq ".cmd" -or $extension -eq ".bat") {',
    '      $resolvedFilePath = "cmd.exe"',
    '      $cmdLine = (@($FilePath) + @($ArgumentList) | ForEach-Object { Quote-CmdArgument $_ }) -join " "',
    '      $resolvedArguments = @("/d", "/s", "/c", $cmdLine)',
    '    } elseif ($extension -eq ".ps1") {',
    '      $resolvedFilePath = "powershell.exe"',
    '      $resolvedArguments = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", $FilePath) + @($ArgumentList)',
    '    } elseif ($extension -eq ".js") {',
    '      $resolvedFilePath = "node.exe"',
    '      $resolvedArguments = @($FilePath) + @($ArgumentList)',
    '    }',
    '    $process = Start-Process -FilePath $resolvedFilePath -ArgumentList $resolvedArguments -NoNewWindow -PassThru -Wait -RedirectStandardOutput $stdoutFile -RedirectStandardError $stderrFile',
    ...
    '      throw ("Native command failed: " + $resolvedFilePath + " " + ($resolvedArguments -join " ") + " :: " + ($stderrText + $stdoutText).Trim())',
    '    }',
    '  } finally {',
    ...
    '}',
    '',
    `Invoke-QuietNative "git" @("clone", "--depth=1", "https://github.com/${repo}.git", "$tmpdir/repo")`,
    ...
    '',
    '# Stop the currently running Windows service before replacing the global package files',
    '& llmproxy service:stop 2>$null | Out-Null',
    '',
    '# npm pack',
    'Invoke-QuietNative "npm" @("pack", "--pack-destination", "$tmpdir")',
    ...
    'Invoke-QuietNative "npm" @("install", "-g", "--force", "$packageFile")',
    '',
    '# Clean up pnpm global if present',
    ...
    '',
    '# Migrate legacy managed config variables to the current schema',
    'Invoke-QuietNative "$newBin" @("config:migrate")',
    '',
    '# Restart service via the new binary',
    'Invoke-QuietNative "$newBin" @("service:restart")',
...
function buildWindowsPersistentInstallScript(options = {}) {
  ...
  const lines = locale === "en"
    ? [
      ...
      "& llmproxy service:stop 2>$null | Out-Null",
      "npm pack $packageRoot --pack-destination $tmpdir 2>&1 | Out-Null",
      "$packageFile = Get-ChildItem (Join-Path $tmpdir '*.tgz') | Select-Object -First 1 -ExpandProperty FullName",
      "if (-not $packageFile) { throw 'Unable to prepare llmproxy package archive for persistent install.' }",
    ...
    : [
      ...
      "npm pack $packageRoot --pack-destination $tmpdir 2>&1 | Out-Null",
      "$packageFile = Get-ChildItem (Join-Path $tmpdir '*.tgz') | Select-Object -First 1 -ExpandProperty FullName",
      "if (-not $packageFile) { throw 'Impossibile preparare l\\'archivio del package llmproxy per l\\'installazione persistente.' }",
...
function resolveInboundProxyApiKey(env = {}) {
  return String(env.LLMPROXY_API_KEY || env.ANTHROPIC_AUTH_TOKEN || "").trim();
}
...
function resolveServiceEnvironment({ env = process.env, paths, dockerComposeFile }) {
  ...
  return {
    ...
    // Keep the host CLI and the inbound production gate on the same key.
    // Older service configs may not have LLMPROXY_API_KEY persisted, while
    // Claude/Pi clients already expose it as ANTHROPIC_AUTH_TOKEN.
    LLMPROXY_API_KEY: String(env.LLMPROXY_API_KEY || env.ANTHROPIC_AUTH_TOKEN || ""),
    ...
    LLMPROXY_PROXY_REGISTRY: paths.proxyRegistryFile,
...
function writePiSettings(options = {}) {
  const cwd = path.resolve(String(options.cwd || process.cwd()));
  const env = options.env || process.env;
  const stdout = options.stdout || process.stdout;
  ...
  stdout.write(`Configurazione PI Agent scritta in ${piDir}/\n`);
  stdout.write(`  ${modelsFile}\n`);
  stdout.write(`  ${settingsFile}\n`);
  stdout.write(`baseUrl: ${baseUrl}\n`);
  return 0;
}

function resolveVscodeChatModelFiles(env = process.env) {
  const platform = String(env.LLMPROXY_PLATFORM_OVERRIDE || process.platform);
  const homeDir = path.resolve(String(env.HOME || os.homedir()));
  const appNames = ["Code", "Code - Insiders", "Code - OSS"];
  const files = [];

  for (const appName of appNames) {
    let filePath;
    if (platform === "win32") {
      const appData = String(env.APPDATA || path.join(homeDir, "AppData", "Roaming"));
      filePath = path.join(appData, appName, "User", "chatLanguageModels.json");
    } else if (platform === "darwin") {
      filePath = path.join(homeDir, "Library", "Application Support", appName, "User", "chatLanguageModels.json");
    } else {
      filePath = path.join(homeDir, ".config", appName, "User", "chatLanguageModels.json");
    }
    files.push(filePath);
  }
...
function resolveVscodeChatModelsFile(env = process.env) {
  return resolveVscodeChatModelFiles(env)[0];
}

function resolveModelsFromTokenStore(tokenStore) {
  if (!tokenStore || typeof tokenStore.listProviders !== "function") return [];
  const providers = tokenStore.listProviders();
  const seen = new Set();
  const models = [];
  for (const provider of providers) {
    const modelName = String(provider.default_model || "").trim();
    const owner = String(provider.provider || provider.id || "unknown").trim().toLowerCase();
...
function toVscodeChatModel(model, openaiUrl) {
  const id = String(model?.id || "").trim();
  if (!id) return null;
  const owner = String(model?.owned_by || "").trim();
  const displayId = id.includes(":") ? id.slice(id.indexOf(":") + 1) : id;
  const isLlmproxyAuto = id.toLowerCase() === "llmproxy";
  return {
    id,
    name: isLlmproxyAuto ? "llmProxy · Auto" : (owner ? `${owner} · ${displayId}` : id),
    // Keep this as a base URL: Agent Host appends /chat/completions itself.
    url: openaiUrl,
    toolCalling: true,
    // Auto can dynamically select one of the configured vision-capable
    // providers when the request includes an image.
    vision: isLlmproxyAuto,
    maxInputTokens: 128000,
    maxOutputTokens: 16000,
  };
}

function ensureLlmproxyAutoModel(models, openaiUrl) {
  const autoModel = toVscodeChatModel({ id: "llmproxy", owned_by: "llmproxy" }, openaiUrl);
  return [autoModel, ...models.filter((model) => String(model?.id || "").toLowerCase() !== "llmproxy")];
}

const VSCODE_CHAT_API_KEY_INPUT = "${input:chat.lm.secret.llmproxy}";

function resolveVscodeChatApiKey(existing) {
  const configuredApiKey = String(existing?.apiKey || "").trim();
  // Preserve the secret-variable name generated by VS Code's Manage Language
  // Models flow. Never copy a raw key from an old/generated config into the
  // file: VS Code must own the secret and inject it at request time.
  if (/^\$\{input:chat\.lm\.secret\.[^}]+\}$/.test(configuredApiKey)) {
    return configuredApiKey;
  }
  return VSCODE_CHAT_API_KEY_INPUT;
}

async function resolveVscodeChatModels({ fetchFn = fetch, openaiUrl, tokenStore } = {}) {
  try {
    const response = await fetchFn(`${openaiUrl}/models`);
    if (response?.ok) {
      const payload = await response.json();
      const models = Array.isArray(payload?.data)
        ? payload.data.map((model) => toVscodeChatModel(model, openaiUrl)).filter(Boolean)
        : [];
      if (models.length > 0) return ensureLlmproxyAutoModel(models, openaiUrl);
    }
  } catch (_error) {
    // Fall back to locally configured provider defaults when the proxy is offline.
  }

  const fallbackModels = resolveModelsFromTokenStore(tokenStore)
    .map((model) => toVscodeChatModel(model, openaiUrl))
    .filter(Boolean);
  return ensureLlmproxyAutoModel(fallbackModels, openaiUrl);
}

async function writeVscodeChatSettings(options = {}) {
  const env = options.env || process.env;
  const stdout = options.stdout || process.stdout;
  const modelsFiles = resolveVscodeChatModelFiles(env);
  ...
  const models = await resolveVscodeChatModels({
  ...
  for (const modelsFile of modelsFiles) {
    ...
    const existingLlmproxyEntry = existing.find((entry) => isLlmproxyVscodeChatEntry(entry));
    const llmproxyEntry = {
      name: "llmProxy",
      vendor: "customendpoint",
      apiKey: resolveVscodeChatApiKey(existingLlmproxyEntry),
      apiType: "chat-completions",
      // Do not set `url` on the provider group. In VS Code's CustomEndpoint
      // provider, a group URL selects endpoint discovery and makes it ignore
      // the explicit `models` array. LLMProxy intentionally supplies the
      // current catalogue as explicit models, each with its own base URL.
      models,
    };
    ...
    const next = existing.filter((entry) => {
      ...
      return !(name === "llmProxy" || name === "llmproxy" || (vendor === "openai" && name.toLowerCase().includes("llmproxy")));
    });
    next.push(llmproxyEntry);

    fs.mkdirSync(path.dirname(modelsFile), { recursive: true });
    fs.writeFileSync(modelsFile, `${JSON.stringify(next, null, 2)}\n`, "utf8");
    stdout.write(`Configurazione VS Code Chat scritta in ${modelsFile}\n`);
  }

  stdout.write(`  URL: ${openaiUrl}\n`);
  stdout.write(`  Modelli: ${models.length} sincronizzati da ${openaiUrl}/models\n`);
  stdout.write(`Riavvia VS Code e seleziona "llmProxy" nel picker della Chat.\n`);
  return 0;
}

function resolveOpenAIClientBaseUrl(options = {}) {
  const env = options.env || process.env;
  const proxyEnvSource = env.LLMPROXY_HOME && options.port == null
    ? { ...env, PORT: "" }
    : env;
...
async function resolveStaticOpenAIClientModels(options = {}) {
  const openaiUrl = resolveOpenAIClientBaseUrl(options);
  const models = await resolveVscodeChatModels({
    fetchFn: options.fetchFn || fetch,
    openaiUrl,
    tokenStore: options.tokenStore,
  });
  return { openaiUrl, models };
}

function resolveOpenCodeConfigFile(env = process.env) {
  const homeDir = path.resolve(String(env.HOME || os.homedir()));
  const configDir = path.join(homeDir, ".config", "opencode");
  const jsoncFile = path.join(configDir, "opencode.jsonc");
  return fs.existsSync(jsoncFile) ? jsoncFile : path.join(configDir, "opencode.json");
}

function resolveContinueConfigFile(env = process.env) {
  const homeDir = path.resolve(String(env.HOME || os.homedir()));
  return path.join(homeDir, ".continue", "config.yaml");
}

function resolveZedSettingsFile(env = process.env) {
  const homeDir = path.resolve(String(env.HOME || os.homedir()));
  const platform = String(env.LLMPROXY_PLATFORM_OVERRIDE || process.platform);
  if (platform === "win32") {
    const appData = String(env.APPDATA || path.join(homeDir, "AppData", "Roaming"));
...
function parseJsoncObject(source, filePath) {
  const errors = [];
  ...
  if (errors.length > 0 || (value != null && (typeof value !== "object" || Array.isArray(value)))) {
    throw new Error(`Configurazione JSON/JSONC non valida: ${filePath}`);
  }
  return value || {};
}

function writeJsoncValues(filePath, updates) {
  const source = fs.existsSync(filePath) ? fs.readFileSync(filePath, "utf8") : "{}\n";
  parseJsoncObject(source, filePath);
  let next = source;
  for (const [jsonPath, value] of updates) {
    const edits = modifyJsonc(next, jsonPath, value, {
      formattingOptions: { insertSpaces: true, tabSize: 2, eol: "\n" },
    });
    next = applyJsoncEdits(next, edits);
  }
...
function toOpenCodeModels(models) {
  return Object.fromEntries(models.map((model) => [model.id, {
    name: model.name,
    limit: { context: model.maxInputTokens, output: model.maxOutputTokens },
  }]));
}

function toZedModels(models) {
...
function toContinueModels(models, openaiUrl) {
  return models.map((model) => ({
    name: model.name,
    provider: "openai",
    model: model.id,
    apiBase: openaiUrl,
    apiKey: "proxy-local",
    capabilities: model.vision === true ? ["tool_use", "image_input"] : ["tool_use"],
    roles: ["chat", "edit", "apply"],
  }));
}

async function writeOpenCodeSettings(options = {}) {
  const stdout = options.stdout || process.stdout;
  const filePath = resolveOpenCodeConfigFile(options.env);
  const { openaiUrl, models } = await resolveStaticOpenAIClientModels(options);
  const existing = fs.existsSync(filePath) ? parseJsoncObject(fs.readFileSync(filePath, "utf8"), filePath) : {};
  const updates = [];
  if (!existing.$schema) updates.push([["$schema"], "https://opencode.ai/config.json"]);
  updates.push([["provider", "llmproxy"], {
    npm: "@ai-sdk/openai-compatible",
    name: "llmProxy",
    options: { baseURL: openaiUrl, apiKey: "proxy-local" },
    models: toOpenCodeModels(models),
  }]);
  writeJsoncValues(filePath, updates);
  stdout.write(`Configurazione OpenCode scritta in ${filePath}\n`);
  stdout.write(`  Modelli: ${models.length} sincronizzati da ${openaiUrl}/models\n`);
  return 0;
}

function readContinueDocument(filePath) {
  const source = fs.existsSync(filePath)
    ? fs.readFileSync(filePath, "utf8")
    : "name: llmProxy\nversion: 1.0.0\nschema: v1\nmodels: []\n";
  const document = YAML.parseDocument(source);
  if (document.errors.length > 0 || !YAML.isMap(document.contents)) {
    throw new Error(`Configurazione YAML non valida: ${filePath}`);
  }
  return document;
}

function getYamlScalar(node, key) {
  const value = node?.get?.(key, true);
  return value && Object.prototype.hasOwnProperty.call(value, "value") ? value.value : undefined;
}

async function writeContinueSettings(options = {}) {
  const stdout = options.stdout || process.stdout;
  const filePath = resolveContinueConfigFile(options.env);
  const { openaiUrl, models } = await resolveStaticOpenAIClientModels(options);
  const document = readContinueDocument(filePath);
  if (!document.has("models")) document.set("models", document.createNode([]));
  const modelList = document.get("models", true);
  if (!YAML.isSeq(modelList)) throw new Error(`La chiave models deve essere una lista YAML: ${filePath}`);
  modelList.items = modelList.items.filter((model) => !(
    getYamlScalar(model, "provider") === "openai"
    && getYamlScalar(model, "apiBase") === openaiUrl
    && getYamlScalar(model, "apiKey") === "proxy-local"
  ));
  for (const model of toContinueModels(models, openaiUrl)) modelList.items.push(document.createNode(model));
  fs.mkdirSync(path.dirname(filePath), { recursive: true });
  fs.writeFileSync(filePath, document.toString({ lineWidth: 0 }), "utf8");
  stdout.write(`Configurazione Continue scritta in ${filePath}\n`);
  stdout.write(`  Modelli: ${models.length} sincronizzati da ${openaiUrl}/models\n`);
  return 0;
}

async function writeZedSettings(options = {}) {
  const stdout = options.stdout || process.stdout;
  const filePath = resolveZedSettingsFile(options.env);
  const { openaiUrl, models } = await resolveStaticOpenAIClientModels(options);
  writeJsoncValues(filePath, [[["language_models", "openai_compatible", "llmproxy"], {
    api_url: openaiUrl,
    available_models: toZedModels(models),
  }]]);
  stdout.write(`Configurazione Zed scritta in ${filePath}\n`);
  stdout.write(`  Imposta la chiave nel keychain Zed o usa LLMPROXY_API_KEY=proxy-local. Modelli: ${models.length}.\n`);
  return 0;
}

function isLlmproxyVscodeChatEntry(entry) {
  if (!entry || typeof entry !== "object") return false;
  const name = String(entry.name || "").trim().toLowerCase();
  const vendor = String(entry.vendor || "").trim().toLowerCase();
  return name === "llmproxy" || (vendor === "openai" && name.includes("llmproxy"));
}

function hasLlmproxyVscodeChatConfig(env = process.env) {
  return resolveVscodeChatModelFiles(env).some((modelsFile) => {
    if (!fs.existsSync(modelsFile)) return false;
    try {
      const entries = JSON.parse(fs.readFileSync(modelsFile, "utf8"));
      return Array.isArray(entries) && entries.some(isLlmproxyVscodeChatEntry);
    } catch (_error) {
      return false;
    }
  });
...
function hasOpenCodeLlmproxyConfig(env = process.env) {
  const filePath = resolveOpenCodeConfigFile(env);
  if (!fs.existsSync(filePath)) return false;
  try {
    return Boolean(parseJsoncObject(fs.readFileSync(filePath, "utf8"), filePath)?.provider?.llmproxy);
  } catch (_error) {
    return false;
  }
}

function hasContinueLlmproxyConfig(env = process.env) {
  const filePath = resolveContinueConfigFile(env);
  if (!fs.existsSync(filePath)) return false;
  try {
    const document = readContinueDocument(filePath);
    const modelList = document.get("models", true);
    return YAML.isSeq(modelList) && modelList.items.some((model) => String(getYamlScalar(model, "name") || "").startsWith("llmProxy · "));
  } catch (_error) {
    return false;
  }
}

function hasZedLlmproxyConfig(env = process.env) {
  const filePath = resolveZedSettingsFile(env);
  if (!fs.existsSync(filePath)) return false;
  try {
    return Boolean(parseJsoncObject(fs.readFileSync(filePath, "utf8"), filePath)?.language_models?.openai_compatible?.llmproxy);
  } catch (_error) {
    return false;
  }
}

async function syncStaticModelClientCatalogsAfterProviderRemoval(options = {}) {
  const env = options.env || process.env;
  if (!options.force && String(env.LLMPROXY_GLOBAL_SERVICE || "").trim() === "1") return [];
  const synced = [];
  if (hasLlmproxyVscodeChatConfig(env)) {
    await writeVscodeChatSettings(options);
    synced.push("VS Code Chat");
  }
  if (hasOpenCodeLlmproxyConfig(env)) {
    await writeOpenCodeSettings(options);
    synced.push("OpenCode");
  }
  if (hasContinueLlmproxyConfig(env)) {
    await writeContinueSettings(options);
    synced.push("Continue");
  }
  if (hasZedLlmproxyConfig(env)) {
    await writeZedSettings(options);
    synced.push("Zed");
  }
  return synced;
}

function writeVscodeClaudeSettings(options = {}) {
...
function resolveConfigScopeFromParsed(parsed, key = "") {
  ...
  const wantsGlobal = Boolean(parsed.flags.global);
  const rawScope = String(parsed.flags.scope || "").trim().toLowerCase();
  if (rawScope && !["project", "global", "service"].includes(rawScope)) {
    throw new Error("Scope non valido. Usa uno tra: project, global, service");
  }
  const explicitScope = wantsProject
    ? "project"
    : rawScope === "project"
      ? "project"
      : wantsGlobal
...
function resolveConfigListScope(parsed, cwd, env = process.env) {
  const explicitScope = resolveConfigScopeFromParsed(parsed);
  if (explicitScope) return explicitScope;
  if (hasLocalClaudeFolder(cwd)) return "";
  return fs.existsSync(getGlobalClaudeSettingsFile(env)) ? "" : "service";
}
...
function formatConfigList(entries, scope = "") {
  ...
  if (normalizedScope === "project") {
    return `Project configuration:\n${formatConfigEntries(entries.filter((entry) => entry.scope === "project"))}\n\nGlobal Claude configuration:\n${formatConfigEntries(entries.filter((entry) => entry.scope === "global"))}`;
  }
  if (normalizedScope === "global") {
    return `Global Claude configuration:\n${formatConfigEntries(entries.filter((entry) => entry.scope === "global"))}`;
  }
...
function buildRestCommandRequest(parsed, options = {}) {
  ...
  switch (command) {
    ...
    case "pi:setup":
      body.projectPath = cwd;
      return { method: "POST", path: "/api/pi/setup", headers, body };
    case "vscode-chat:setup":
      body.projectPath = cwd;
      return { method: "POST", path: "/api/vscode/chat/setup", headers, body };
    case "vscode-claude:setup":
      if (parsed.flags.model) body.model = String(parsed.flags.model).trim();
      body.projectPath = cwd;
      return { method: "POST", path: "/api/vscode/claude/setup", headers, body };
    case "model:set":
    ...
    case "provider:update": {
      ...
      const patch = {};
      if (flags.vision !== undefined && flags.vision !== null && flags.vision !== "") patch.vision = flags.vision;
      if (flags["free-model"] !== undefined && flags["free-model"] !== null && flags["free-model"] !== "") patch.free_model = flags["free-model"];
      if (flags.name !== undefined && flags.name !== null && flags.name !== "") patch.name = String(flags.name).trim();
      return {
        method: "PATCH",
        path: `/api/providers/${encodeURIComponent(providerId)}`,
        headers,
        body: { patch },
      };
    }
...
async function tryRunViaRest(parsed, options = {}) {
  ...
  if ([
    ...
    "service:restart",
    "service:runtime",
    // These commands write user-owned local configuration files. Never run
    // them inside a remote or Docker-hosted proxy process.
    "opencode:setup",
    "continue:setup",
    "zed:setup",
  ].includes(parsed.command)) return null;
  ...
  try {
    ...
    const runtimeApiKey = resolveInboundProxyApiKey(options.env || {});
    if (runtimeApiKey && !requestHeaders["x-api-key"]) {
      requestHeaders["x-api-key"] = runtimeApiKey;
    }
    ...
    // The production platform runtime intentionally does not expose the
    // standalone /api/* control-plane routes.  In that case the CLI must
    // continue with its local implementation instead of swallowing the
    // REST error and returning an empty prompt.
    ...
    if (!data.output && !data.error && payload?.message) {
      options.stderr.write(`${String(payload.message).trimEnd()}\n`);
    }
    const exitCode = Number.isFinite(Number(payload?.exitCode)) ? Number(payload.exitCode) : (payload?.success ? 0 : 1);
    if (exitCode === 0 && parsed.command === "provider:remove") {
      try {
        await syncStaticModelClientCatalogsAfterProviderRemoval({
          env: options.env,
          dataRoot: options.dataRoot,
          stdout: options.stdout,
          tokenStore: options.tokenStore,
        ...
        options.stderr.write(`Provider rimosso, ma la sincronizzazione VS Code Chat non e' riuscita: ${error.message}\n`);
      }
    }
    return exitCode;
  } catch {
...
async function withTemporaryProviderProxy(providerStore, provider, proxyUrl, fn) {
  if (!providerStore || !provider || !provider.id || !proxyUrl) return fn(provider);

  const originalProvider = providerStore.getProvider(provider.id);
  if (!originalProvider) return fn(provider);

  providerStore.saveProvider(provider.id, {
    ...originalProvider,
    proxy_url: proxyUrl,
  ...
  try {
    ...
    providerStore.saveProvider(provider.id, originalProvider, { name: originalProvider.name });
  }
}
...
async function probeProviderForTest({ fetchFn, proxyBaseUrl, provider, sleepFn, logsDir, headers = {} }) {
  ...
  if (summary && !requestSummaryMatchesProvider(summary, provider)) {
    summary = null;
  }
  if (!visibleAssistantText && !(summary?.success && requestSummaryMatchesProvider(summary, provider))) {
    summary = await waitForLatestRequestSummary(logsDir, {
      sinceIso: providerStartedAtIso,
      sleepFn,
      matcher: (entry) => entry?.success === true && requestSummaryMatchesProvider(entry, provider),
    });
  }
...
function ensureDockerServiceRunning({ commandRunner, env, paths, targetPlatform, forceRecreate = false }) {
  ...
  const alreadyRunning = isDockerServiceRunning(commandRunner, composeFile, serviceName, spawnOptions);
  if (alreadyRunning && !forceRecreate) {
    return { ok: true, checked: true, restarted: false, composeLabel: dockerCompose.label };
  }
  const upArgs = forceRecreate
    ? ["up", "-d", "--build", "--force-recreate", serviceName]
    : ["up", "-d", "--build", serviceName];
  const upResult = runDockerCompose(commandRunner, composeFile, upArgs, { invocation: dockerCompose, spawnOptions });
...
async function runCli(argv, options = {}) {
  ...
  const tokenStore = options.tokenStore || createTokenStore({ filePath: paths.tokenFile, secret: env.LLMPROXY_SECRET || null });
  ...
  if (parsed.command === "models:list") {
    const showAll = Boolean(parsed.flags.all || parsed.flags["all-providers"]);
    if (showAll) {
      ...
      if (providers.length === 0) {
        stderr.write("Nessun provider configurato. Usa llmproxy provider:add <provider> per aggiungerne uno.\n");
        return 1;
      }
      const seenModels = new Set();
      const entries = providers.map((p, idx) => {
        ...
        const intelligence = String(p.intelligence_score || p.coding || "").trim();
        const credit = String(p.credit || "").trim();
        const bench = String(p.bench || "").trim();
        return { idx: idx + 1, key, providerId, modelId, intelligence, credit, bench };
      }).filter((e) => {
        if (seenModels.has(e.key)) return false;
        seenModels.add(e.key);
        return true;
      });
      for (const e of entries) {
        const extras = [];
        if (e.credit && e.credit !== "n/a") extras.push(`credit=${e.credit}`);
        if (e.intelligence) extras.push(`coding=${e.intelligence}`);
        if (e.bench && !e.bench.includes("errore")) extras.push(`bench=${e.bench}`);
        const extra = extras.length > 0 ? `  (${extras.join(", ")})` : "";
        stdout.write(`${e.idx}. ${e.key}${extra}\n`);
      }
    }
    return 0;
  }
  ...
  if (parsed.command === "test") {
    ...
    const targetProviderArg = inferenceMode ? String(parsed.args[0] || "").trim() : "";
    let targetProvider = null;
    let targetModelOverride = null;
    if (targetProviderArg) {
      const numericIndex = /^\d+$/.test(targetProviderArg) ? parseInt(targetProviderArg, 10) : -1;
      if (numericIndex >= 1 && numericIndex <= providers.length) {
        targetProvider = providers[numericIndex - 1];
      } else {
        const byId = providers.find((p) => String(p.id || "").toLowerCase() === targetProviderArg.toLowerCase());
        if (byId) {
          targetProvider = byId;
        } else {
          // Se non è un id/indice, trattalo come nome di modello (es. tencent/hy3:free, opencode-bacin2, qwen)
          const byModel = providers.find(
            (p) => String(p.effective_model || p.default_model || "").toLowerCase() === targetProviderArg.toLowerCase(),
          );
          if (byModel) {
            targetProvider = byModel;
            targetModelOverride = targetProviderArg;
          }
        }
      }
      if (!targetProvider) {
        stderr.write(`Provider o modello non trovato: ${targetProviderArg}\nUsa llmproxy provider:list per vedere i provider configurati.\n`);
        return 1;
      }
    }
    ...
    const commandCwd = path.resolve(String(options.cwd || process.cwd()));
    const projectSettings = resolveClaudeProjectSettings(
      commandCwd,
      { env },
    );
    const effectiveStatsKey = String(getScopeValue({
      key: "LLMPROXY_LLM_STATS_API_KEY",
      cwd: commandCwd,
      serviceConfigFile: paths.serviceConfigFile,
      env,
    }).value || "").trim();
    if (!effectiveStatsKey) {
      stderr.write(`${LLM_STATS_API_KEY_REQUIRED_MESSAGE}\n`);
      return 1;
    }

    let failures = 0;
    const proxyApiKey = resolveInboundProxyApiKey(env);
    const proxyAuthHeaders = proxyApiKey
      ? { "x-api-key": proxyApiKey }
      : {};

    if (inferenceMode) {
        const infProxyFlag = String(parsed.flags.proxy || "").trim();
  const infProviderFlag = String(parsed.flags.provider || "").trim();
  const infFetch = infProxyFlag ? makeProxyFetch(fetchFn, infProxyFlag) : fetchFn;
  const infProxyLabel = infProxyFlag ? ` via proxy ${infProxyFlag}` : "";

  // Se --provider flag specificato, sovrascrive targetProvider
  let infTargetProvider = targetProvider;
  if (infProviderFlag && !targetProvider) {
    const byId = providers.find((p) => String(p.id || "").toLowerCase() === infProviderFlag.toLowerCase());
    const byName = providers.find((p) => String(p.name || "").toLowerCase() === infProviderFlag.toLowerCase());
    infTargetProvider = byId || byName || null;
    if (!infTargetProvider) {
      stderr.write(`Provider non trovato: ${infProviderFlag}\n`);
      return 1;
    }
  }
  const startedAtIso = new Date().toISOString();
      ...
      try {
        ...
        const response = await runLocalProxyTestRequest(
          ...
          `${proxyBaseUrl}/v1/messages`,
          {
            ...
            body: JSON.stringify({
              stream: false,
              max_tokens: 256,
              ...(targetProvider ? { provider: targetProvider.id } : {}),
              ...(targetModelOverride
                ? { model: targetModelOverride }
                : (targetProvider ? { model: targetProvider.effective_model || targetProvider.default_model } : {})),
              messages: [
                {
                  role: "user",
        ...
        if (!response.ok) {
          stdout.write(`inference: fail HTTP ${response.status}${infProxyLabel}\n`);
          return 1;
        }
        ...
        stdout.write(`inference: ok${finalProvider || finalModel ? ` (${[finalProvider, finalModel].filter(Boolean).join(" | ")})` : ""}${infProxyLabel} [${elapsedMs}ms]\n`);
        if (allProviders) {
            ...
            try {
              ...
              if (probe.ok) {
                workingFallbacks.push({
                  ...
                  degraded: probe.degradedStatus || null,
                });
              } else {
          ...
          workingFallbacks.forEach((provider, index) => {
            const degradedLabel = provider.degraded ? ` degraded HTTP ${provider.degraded}` : "";
            stdout.write(`${formatOrdinalLabel(index + 1)} fallback: ${provider.id}${degradedLabel} | ${provider.model || "missing"}\n`);
          });
    ...
    for (const provider of selectedTargets) {
      try {
        ...
        const degradedLabel = probe.degradedStatus ? ` degraded HTTP ${probe.degradedStatus}` : "";
        stdout.write(`${provider.id}: ok${degradedLabel} (${probe.displayModel || probe.model || provider.default_model || ""})\n`);
      } catch (error) {
        const model = provider.default_model || (String(provider.provider || "copilot") === "copilot" ? DEFAULT_COPILOT_MODEL : "");
  ...
  if (parsed.command === "pi:setup") {
    try {
      return writePiSettings({ cwd: options.cwd, env, stdout, dataRoot: paths.dataRoot });
    } catch (error) {
      stderr.write(`${error.message}\n`);
      return 1;
    }
  ...
  if (parsed.command === "vscode-chat:setup") {
    try {
      return writeVscodeChatSettings({
        env,
        stdout,
        dataRoot: paths.dataRoot,
        fetchFn,
  ...
  if (parsed.command === "opencode:setup" || parsed.command === "continue:setup" || parsed.command === "zed:setup") {
    const writers = {
      "opencode:setup": writeOpenCodeSettings,
      "continue:setup": writeContinueSettings,
      "zed:setup": writeZedSettings,
    };
    try {
      return await writers[parsed.command]({
        env,
        stdout,
        dataRoot: paths.dataRoot,
        fetchFn,
  ...
  if (parsed.command === "vscode-claude:setup") {
    try {
      const models = await resolveAvailableCopilotModels({
        tokenStore,
        fetchFn,
      ...
      return writeVscodeClaudeSettings({
        cwd: options.cwd,
        env,
        stdout,
  ...
  if (parsed.command === "config:list") {
    try {
      const scope = resolveConfigListScope(parsed, options.cwd, { ...process.env, ...env });
  ...
  if (parsed.command === "config:get") {
    try {
      const key = String(parsed.args[0] || "").trim();
      if (!key) {
        throw new Error("Uso: llmproxy config:get <key> [--project|--service|--scope <project|global|service>]");
      }
  ...
  if (parsed.command === "config:set") {
    try {
      ...
      if (!key || parsed.args.length < 2) {
        throw new Error("Uso: llmproxy config:set <key> <value> [--project|--service|--scope <project|global|service>]");
      }
      const scope = resolveConfigScopeFromParsed(parsed, key);
  ...
  if (parsed.command === "config:unset") {
    try {
      const key = String(parsed.args[0] || "").trim();
      if (!key) {
        throw new Error("Uso: llmproxy config:unset <key> [--project|--service|--scope <project|global|service>]");
      }
  ...
  if (parsed.command === "update") {
    ...
    try {
      ...
      const versionComparison = remoteVersion ? compareReleaseVersions(currentVersion, remoteVersion) : null;
      if (remoteVersion && (remoteVersion === currentVersion || (versionComparison != null && versionComparison >= 0))) {
        if (locale === "en") {
    ...
    if (result?.status === 0) {
      ...
      if (versionMatch?.[1]) {
        ...
        const releaseNotes = releaseNotesMatch?.[1]?.trim() || formatReleaseNotes(currentVersion, locale, { packageRoot: paths.packageRoot });
  ...
  if (parsed.command === "provider:add") {
    const rawProviderArg = String(parsed.args[0] || "").trim();
    const providerId = normalizeKnownProviderId(rawProviderArg);
    const apiKey = String(parsed.flags["api-key"] || "").trim();
    const apiKeyProviderKind = resolveApiKeyProviderKind(rawProviderArg, parsed.flags.provider);
    const providerKind = apiKey ? apiKeyProviderKind : providerId;
    const instanceId = apiKey && providerId && providerId !== providerKind ? providerId : providerKind;
    const providerName = String(parsed.flags.name || getKnownProvider(providerKind || providerId).displayName || instanceId || providerKind || providerId || "").trim();
    ...
    const endpointVariant = resolveQwenEndpointVariant(providerKind || providerId, parsed.flags.plan, apiKey);
    ...
    const providerInfo = getKnownProvider(providerKind || providerId);
    if (apiKey && !apiKeyProviderKind) {
      stderr.write(`Provider API-key non supportato: ${providerId}. Usa un id canonico oppure --provider <tipo>, ad esempio: llmproxy provider:add opencode-bacin --provider opencode --api-key <key> --model <model> --vision false\n`);
      return 1;
    }
    ...
    if (providerInfo.auth === "api_key") {
      ...
      if (providerKind === "qwen" && parsed.flags.plan && !explicitQwenPlan) {
        stderr.write("Per `qwen` usa --plan subscription oppure --plan payg.\n");
        return 1;
      }
      if (!defaultModel) {
        stderr.write(`Il provider ${providerKind} richiede --model per salvare e verificare il modello di default.\n`);
        return 1;
      }
      if (visionFlag === undefined || visionFlag === null || visionFlag === "") {
        stderr.write(`Il provider ${providerKind} richiede --vision <true|false>. Esempio: llmproxy provider:add ${instanceId || providerKind} --api-key <key> --model ${defaultModel} --vision true\n`);
        return 1;
      }
      ...
      const probe = await probeApiKeyProviderModel({ provider: { provider: providerKind, endpoint_variant: endpointVariant }, apiKey, model: defaultModel, fetchFn, proxyUrl });
      if (!probe.ok) {
        stderr.write(`Test provider fallito per ${providerKind}/${defaultModel}: ${probe.status || "network"} ${probe.error || ""}\n`);
        return 1;
      }
      const allProviders = providerStore.listProviders();
      const sameKindProviders = allProviders.filter((p) => String(p.provider || "").toLowerCase() === providerKind);
      const hasExplicitInstanceId = Boolean(providerId && providerId !== providerKind);
      const hasExplicitName = Boolean(providerName && providerName !== (getKnownProvider(providerKind || providerId)?.displayName || ""));
      const nameBasedId = hasExplicitName && !hasExplicitInstanceId
        ? providerName.toLowerCase().replace(/[^a-z0-9_-]/g, "-").replace(/^-+|-+$/g, "")
        : null;
      const existingWithSameModel = (hasExplicitInstanceId || nameBasedId) ? null : sameKindProviders.find((p) => p.default_model === defaultModel);
      const targetInstanceId = existingWithSameModel
        ? existingWithSameModel.id
        : (hasExplicitInstanceId ? providerId : (nameBasedId || (sameKindProviders.length > 0 ? defaultModel : providerKind)));
      ...
      // Fetch and store coding score from live API (once at provider insertion)
      fetchCodingScore(defaultModel, fetchFn).then((score) => {
        if (typeof score === "number" && Number.isFinite(score)) {
          providerStore.saveProvider(targetInstanceId, { coding_score: score }, { name: providerName || providerId });
        }
        return null;
      }).catch(() => {});
      const planSuffix = providerKind === "qwen" ? `, plan: ${formatQwenPlanLabel(endpointVariant)}` : "";
      stdout.write(`Provider configurato con API key: ${targetInstanceId} (default model: ${defaultModel}, vision: ${visionEnabled}, free: ${freeModel}${planSuffix}).\n`);
      return 0;
    }
    ...
    // Fetch and store coding score after successful login
    const copilotModel = defaultModel || DEFAULT_COPILOT_MODEL;
    fetchCodingScore(copilotModel, fetchFn).then((score) => {
      if (typeof score === "number" && Number.isFinite(score)) {
        providerStore.saveProvider(providerId, { coding_score: score }, { name: providerName || providerId });
      }
      return null;
    }).catch(() => {});
  ...
  if (parsed.command === "provider:key") {
    ...
    // Fetch and store coding score (model scores are static — fetch once)
    fetchCodingScore(nextDefaultModel, fetchFn).then((score) => {
      if (typeof score === "number" && Number.isFinite(score)) {
        providerStore.saveProvider(effectiveId, { coding_score: score }, { name: existing?.name || providerInfo.displayName || providerId });
      }
      return null;
    }).catch(() => {});
  ...
  if (parsed.command === "provider:list") {
    ...
    const projectSettings = resolveClaudeProjectSettings(path.resolve(String(options.cwd || process.cwd())), { env });
    ...
    // Spinner: segnala che il benchmarking dei provider è in corso
    const benchSpinner = (process.stdout.isTTY)
      ? startSpinner("We are benchmarking each provider/model for you: please wait")
      : null;
    const providersWithCredit = await Promise.all(
      effectiveProviders.providers.map(async (provider) => {
        const [coding_info, credit_info, price_info, reorder_info] = await Promise.all([
          fetchProviderCodingInfo(provider, fetchFn, codingCache),
          fetchProviderCreditInfo(provider, fetchFn, creditCache),
          fetchProviderPriceInfo(provider, fetchFn, priceCache),
          Promise.resolve(reorderingStore.scores?.[provider.id] || null),
        ]);
        let benchMetric = reorder_info?.speed != null
          ? `${reorder_info.speed} ms`
          : "n/a";
        // Fallback: fetch speed probe live if not in reorder info
        if (benchMetric === "n/a") {
          const speedResult = await fetchSpeedMetric(provider, probeFn, fetchFn);
          if (typeof speedResult === "number") {
            benchMetric = `${speedResult} ms`;
          } else if (typeof speedResult === "string") {
            benchMetric = speedResult; // error message like "errore 429"
          } else {
            benchMetric = "n/a";
          }
        }
        // Add benchmark_score field for display
        const benchmarkScore = reorder_info?.power != null
          ? reorder_info.power
          : (await fetchBenchmarkMetric(provider, fetchFn));
        return {
          ...provider,
          coding_info,
          credit_info,
          price_info,
          reorder_info,
          bench_metric: benchMetric,
          benchmark_score: benchmarkScore,
        };
      }),
    );
    if (benchSpinner) stopSpinner(benchSpinner);
  ...
  if (parsed.command === "provider:test") {
    ...
    const visionMode = parsed.flags["vision"] === true;
    const allProxiesMode = parsed.flags["all-proxies"] === true;
    const forcedProxyUrl = String(parsed.flags.proxy || "").trim();
    const forcedProviderId = String(parsed.flags.provider || "").trim();
    const proxyInferenceMode = allProxiesMode || Boolean(forcedProxyUrl);
    let passCount = 0;
    let failCount = 0;
    let skipCount = 0;
    ...
    let visionKeywords = [];
    if (forcedProxyUrl) {
      stdout.write(`Test di inferenza con proxy forzato: ${forcedProxyUrl}\n\n`);
      proxiesList = [{ url: forcedProxyUrl }];
    } else if (allProxiesMode) {
      ...
      stdout.write("Test di inferenza provider × proxy...\n\n");
    } else if (visionMode) {
      // Test visione: leggi l'immagine di test dal file assets
      ...
      visionPrompt = "Descrivi questa immagine. Quali colori vedi e dove sono posizionati?";

      // Parole chiave che indicano visione effettiva
      visionKeywords = ["rosso", "red", "blu", "blue", "colore", "color", "immagine", "image", "alto", "top", "basso", "bottom", "sopra", "above", "sotto", "below"];
      stdout.write("Test visione provider...\n\n");
    } else {
      // Default: inferenza testuale semplice
      stdout.write("Test inferenza provider...\n\n");
    }
    ...
    for (const provider of providers) {
      ...
      // Filtra per --provider esplicito
      if (forcedProviderId && provider.id !== forcedProviderId && (provider.name || "").toLowerCase() !== forcedProviderId.toLowerCase()) {
        continue;
      }

      const proxyUrls = proxyInferenceMode
        ? proxiesList.map((p) => p.url)
        : [""];

      if (proxyInferenceMode) {
        stdout.write(`${providerName} (${model})\n`);
      }

      for (const proxyUrl of proxyUrls) {
        const providerStartedAtIso = new Date().toISOString();
        if (proxyInferenceMode) {
          const proxyHost = proxyUrl ? new URL(proxyUrl).hostname : "direct";
          stdout.write(`  ${proxyHost}\n`);
        } else if (visionMode) {
          ...
          // Default text inference: simple label
          const proxyLabel = proxyUrl ? ` (via ${new URL(proxyUrl).hostname})` : "";
          stdout.write(`🔍 ${providerName} (${model})${proxyLabel}\n`);
        }

        try {
          const baseUrl = getProxyBaseUrl({ env, dataRoot: paths.dataRoot });
          const testTokenPrefix = proxyInferenceMode ? "llmproxy-proxy-test-" : "llmproxy-test-";
          // Determine messages based on mode
          const buildMessages = (providerUnderTest) => visionMode
            ? [
                {
                  role: "user",
                  content: [
            ...
            : [
                {
                  role: "user",
                  content: [{ type: "text", text: `Rispondi solo: ${testTokenPrefix}${providerUnderTest.id}` }],
                },
              ];
          const response = await withTemporaryProviderProxy(providerStore, provider, proxyUrl, async (providerUnderTest) => (
            runLocalProxyTestRequest(
              fetchFn,
              `${baseUrl}/v1/messages`,
              {
                method: "POST",
                headers: {
                  "content-type": "application/json",
                  ...(resolveInboundProxyApiKey(env)
                    ? { "x-api-key": resolveInboundProxyApiKey(env) }
                    : {}),
                },
                body: JSON.stringify({
                  provider: providerUnderTest.id,
                  model,
                  stream: false,
                  max_tokens: 150,
                  messages: buildMessages(providerUnderTest),
                }),
              },
              sleep,
            )
          ...
          if (!response.ok) {
            ...
            if (visionMode && vision === false) {
              stdout.write(`  ✅ PASS - Visione correttamente disabilitata (errore HTTP ${response.status})\n`);
              passCount++;
              continue;
            }
            if (proxyInferenceMode) {
              stdout.write(`    FAIL HTTP ${response.status}: ${errorText.replace(/\s+/g, " ").slice(0, 180)}\n`);
            } else {
          ...
          const visibleAssistantText = sanitizeTestAssistantText(assistantText);
          const summary = proxyInferenceMode
            ? await waitForLatestRequestSummary(paths.logsDir, {
              sinceIso: providerStartedAtIso,
              sleepFn: sleep,
              matcher: (entry) => entry?.success === true && requestSummaryMatchesProvider(entry, provider),
            })
          ...
          if (proxyInferenceMode) {
            if (!visibleAssistantText && !summary?.success) {
              stdout.write("    FAIL risposta vuota\n");
              failCount++;
              continue;
            }
            const finalModel = String(summary?.finalModel || payload?.model || model || "").trim();
            stdout.write(`    OK ${finalModel}\n`);
            if (visibleAssistantText) {
              stdout.write(`    ${visibleAssistantText.replace(/\s+/g, " ").slice(0, 160)}\n`);
            } else {
              stdout.write("    risposta testuale non visibile, ma inferenza completata correttamente\n");
            }
            passCount++;
            continue;
          }
          ...
          if (visionMode) {
            // Vision mode: check response for vision indicators
            const reasoningText = payload.reasoning_content || "";
          ...
          // Default text inference mode
          if (!visibleAssistantText) {
            stdout.write(`  ❌ Risposta vuota\n`);
            failCount++;
            continue;
          }
          const finalModel = String(payload?.model || model || "").trim();
          stdout.write(`  ✅ PASS (${finalModel})\n`);
          stdout.write(`     ${visibleAssistantText.replace(/\s+/g, " ").slice(0, 160)}\n`);
          passCount++;
        } catch (error) {
          const isProxyError = String(error.message || "").includes("fetch failed");
          if (isProxyError && allProxiesMode) {
            stdout.write(proxyInferenceMode
              ? "    Proxy non raggiungibile\n"
              : "  ℹ️  Proxy non raggiungibile (ignorato in modalità probe)\n");
    ...
    if (proxyUnreachableCount > 0) {
      stdout.write(`, ${proxyUnreachableCount} proxy non raggiungibili (non contati come fallimento)`);
    }
    stdout.write("\n");
  ...
  if (parsed.command === "provider:update") {
    const providerId = String(parsed.args[0] || "").trim();
    if (!providerId) {
      stderr.write("Uso: llmproxy provider:update <id> [--vision <true|false>] [--free-model <true|false>] [--name <name>]\n");
      return 1;
    }
    const patch = {};
    const visionRaw = parsed.flags.vision;
    if (visionRaw !== undefined && visionRaw !== null && visionRaw !== "") {
      if (visionRaw !== "true" && visionRaw !== true && visionRaw !== "false" && visionRaw !== false) {
        stderr.write(`--vision deve essere 'true' oppure 'false'. Valore ricevuto: '${visionRaw}'\n`);
        return 1;
      }
      patch.vision = visionRaw === "true" || visionRaw === true;
    }
    const freeRaw = parsed.flags["free-model"];
    if (freeRaw !== undefined && freeRaw !== null && freeRaw !== "") {
      const parsedFree = parseOptionalBooleanFlag(freeRaw, { defaultValue: null, allowBareTrue: true });
      if (parsedFree === null) {
        stderr.write(`--free-model deve essere 'true' oppure 'false'. Valore ricevuto: '${freeRaw}'\n`);
        return 1;
      }
      patch.free_model = parsedFree;
    }
    const nameRaw = parsed.flags.name;
    if (nameRaw !== undefined && nameRaw !== null && nameRaw !== "") {
      patch.name = String(nameRaw).trim();
    }
    if (Object.keys(patch).length === 0) {
      stderr.write("Nessun campo da aggiornare. Specifica almeno uno tra --vision, --free-model, --name.\n");
      return 1;
    }
    try {
      const provider = providerStore.updateProvider(providerId, patch);
      stdout.write(`Provider aggiornato: ${provider.id}\n`);
      if (patch.vision !== undefined) stdout.write(`  vision: ${provider.vision}\n`);
      if (patch.free_model !== undefined) stdout.write(`  free: ${provider.free_model}\n`);
      if (patch.name) stdout.write(`  name: ${provider.name}\n`);
      return 0;
    } catch (error) {
      stderr.write(`${error.message}\n`);
      return 1;
  ...
  if (parsed.command === "provider:remove") {
    ...
    if (!existing) {
      stderr.write(`Provider non trovato: ${providerId}\n`);
      return 1;
    }
    ...
    try {
      ...
      stderr.write(`Provider rimosso, ma la sincronizzazione VS Code Chat non e' riuscita: ${error.message}\n`);
    }
    return 0;
  }
  ...
  if (parsed.command === "proxy:add") {
    const url = String(parsed.args[0] || "").trim();
    const proxyName = String(parsed.flags.name || "").trim();
    if (!url) {
      stderr.write("URL proxy richiesto. Uso: llmproxy proxy:add <url> [--name <nome>]\n");
      return 1;
    }
    try {
      const proxy = proxyStore.addProxy(url, proxyName || undefined);
      const nameSuffix = proxy.name ? ` (${proxy.name})` : "";
      stdout.write(`Proxy aggiunto: ${proxy.id}${nameSuffix} (${proxy.url})\n`);
      return 0;
    } catch (err) {
  ...
  if (parsed.command === "proxy:list") {
    ...
    proxies.forEach((p, i) => {
      const nameTag = p.name ? ` [${p.name}]` : "";
      stdout.write(`  ${i + 1}. ${p.id}${nameTag} (${p.url})\n`);
    });
    return 0;
  }
  ...
  if (parsed.command === "proxy:test") {
    ...
    for (let i = 0; i < proxies.length; i++) {
      ...
      try {
        ...
        // Prova piu' host per resilienza a blocchi/rate-limit
        const testUrls = [
        ...
        for (const url of testUrls) {
          try {
            const resp = await probeFetch(url, { signal: AbortSignal.timeout(10000) });
  ...
  if (parsed.command === "stats") {
    try {
      ...
      const useColor = shouldUseColorOutput(stdout, env);
      stdout.write(`${formatStatsReport(result.stats, result.source, useColor)}\n`);
      return 0;
    } catch (error) {
      stderr.write(`${error.message}\n`);
      return 1;
  ...
  if (parsed.command === "stats:reset") {
    const unexpectedFlags = Object.keys(parsed.flags || {});
    if (unexpectedFlags.length > 0) {
      stderr.write(`Flag non supportato per stats:reset: --${unexpectedFlags[0]}. Il comando non accetta flag; uso: llmproxy stats:reset\n`);
      return 1;
    }
    try {
      const truncated = [];
  ...
  if (parsed.command === "service:start") {
    ...
    if (serviceManager.kind === "windows" && typeof serviceManager.status === "function") {
      const postInstallStatus = serviceManager.status();
      if (!postInstallStatus.ok || !postInstallStatus.active) {
        stderr.write("Il servizio Windows e' stato registrato ma non risulta attivo dopo l'avvio.\n");
        return 1;
      }
    }
  ...
  if (parsed.command === "service:restart") {
    ...
    if (serviceManager.kind === "windows" && typeof serviceManager.status === "function") {
      const postRestartStatus = serviceManager.status();
      if (!postRestartStatus.ok || !postRestartStatus.active) {
        stderr.write("Il servizio Windows e' stato riavviato ma non risulta attivo.\n");
        return 1;
      }
    }
