...
Autenticazione:
  llmproxy login                           ${s("login")}  alias legacy di provider:add copilot; mantienilo solo per compatibilita'
  llmproxy logout                          ${s("logout")}  rimuove i token locali; usalo per resettare l'accesso corrente
...
Provider:
  llmproxy provider:add <id> ...           ${s("provider:add")}  aggiunge un provider noto (Copilot via login; gli altri via API key)
...
Smart Router:
  llmproxy smart:add --provider <p> --model <m> --api-key <k>  ${s("smart:add")}  aggiunge il classifier LLM per il routing intelligente
  llmproxy smart:status                   ${s("smart:status")}  mostra stato dello smart router e classifier configurato
  llmproxy smart:test                     ${s("smart:test")}  simula il routing su scenari simple/moderate/complex
  llmproxy smart:refresh                  ${s("smart:refresh")}  invalida la cache availability dei provider
