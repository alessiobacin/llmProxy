...
const COMMAND_HELP = {
  ...
  "provider:test": {
    usage: "llmproxy provider:test [--all-proxies] [--proxy <url>] [--provider <id>]",
    description: "Testa i provider configurati. In modalita' standard verifica la visione immagini; con --all-proxies o --proxy esegue una vera inferenza testo su ogni combinazione provider × proxy.",
    when: "Usalo dopo aver configurato i provider per verificare la visione immagini oppure, con i flag proxy, per fare uno smoke test reale di inferenza attraverso i proxy registrati.",
...
Provider:
  ...
  llmproxy provider:test [--all-proxies]   ${s("provider:test")}  testa la visione; con --all-proxies fa inferenza reale su ogni proxy
      ...
      for (const proxyUrl of proxyUrls) {
        ...
        try {
            ...
            runLocalProxyTestRequest(
              ...
              `${baseUrl}/v1/messages`,
              {
                ...
                body: JSON.stringify({
                  ...
                  stream: false,
                  max_tokens: 150,
                  messages: proxyInferenceMode
                    ? [
                      {
                        role: "user",
                        content: [{ type: "text", text: `Rispondi solo: llmproxy-proxy-test-${providerUnderTest.id}` }],
                      },
                    ]
                    : [
                      {
          ...
          if (!response.ok) {
            ...
            if (!proxyInferenceMode && vision === false) {
