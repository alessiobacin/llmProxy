...
const CONFIG_SPECS = [
  ...
  { key: "LLMPROXY_PROVIDER_CREDIT_INLINE",     scope: "service", restartRequired: false, hotReloadable: true },
  ...
  { key: "LLMPROXY_API_KEY",                    scope: "service", restartRequired: true, hotReloadable: false },
  ...
  { key: "LLMPROXY_INTENT_ESCALATION",          scope: "service", restartRequired: true, hotReloadable: false },
  { key: "LLMPROXY_INTENT_ESCALATION_GAP",      scope: "service", restartRequired: true, hotReloadable: false },
  { key: "LLMPROXY_INTENT_INFO_LINE",           scope: "service", restartRequired: true, hotReloadable: false },
];
...
function inferScopeFromKey(key, explicitScope) {
  ...
  if (spec.scope === "service" && !normalizedScope) {
    throw new Error(`Questa variabile richiede lo scope esplicito: usa --scope service`);
  }
  if (!normalizedScope) return spec.scope;
  if (normalizedScope === "global") {
    if (spec.scope !== "project") {
      throw new Error(`Questa variabile può essere impostata soltanto nello scope '${spec.scope}'`);
    }
    return "global";
  }
...
function getServiceDefaultValues(options = {}) {
  ...
  return {
    ...
    LLMPROXY_API_KEY: String(runtimeEnv.LLMPROXY_API_KEY || ""),
    ...
    LLMPROXY_PROVIDER_CREDIT_INLINE: String(runtimeEnv.LLMPROXY_PROVIDER_CREDIT_INLINE || "1"),
