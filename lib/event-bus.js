...
function createEventBusSink({ url, fetchFn = fetch, notifier, healthCheckIntervalMs } = {}) {
  ...
  async function probe() {
    ...
    const result = await healthProbe(baseUrl, _fetch);
    _available = result.ok;

    if (_notifier) {
      if (wasAvailable && !_available) {
        const probeError = result.error || new Error("health check failed");
        _notifier.notifyUnreachable("event-bus", baseUrl, probeError);
