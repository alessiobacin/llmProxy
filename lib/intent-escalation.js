...
const { getCodingScore } = require("./model-capabilities");
...
INTENT: debug deploy | SAME: yes (user said "still failing" after "fix deployment")`,
   ...
   * Only increments count when a NEW human message arrives (not on LLM tool calls).
   ...
   * @param {string} [userMessage] - The raw user message text for dedup.
   * @returns {{ count: number, escalated: boolean, escalationModel: string|null }}
   */
  track(intent, currentModel, availableProviders, continuation = null, userMessage = null) {
    ...
    // Find current conversation by matching recent entries (no time limit)
    ...
    if (sorted.length > 0) {
      ...
      // Dedup: skip counting if this is the same user message as the last one
      // (Claude Code makes multiple inference calls per human turn)
      if (userMessage != null && current.lastUserMessage === userMessage) {
        // Same user message already counted — don't increment
      } else if (current.intent === intent) {
        current.count += 1;
      } else {
      ...
      // Store the user message for next dedup
      if (userMessage != null) {
        current.lastUserMessage = userMessage;
      }
    } else {
      // New conversation
  ...
  _findEscalationModel(currentModel, providers, excludeModels = []) {
    // Prefer stored coding_score from provider data, fallback to static getCodingScore
    const currentScore = getCodingScore(currentModel);
    ...
    for (let gap = startGap; gap >= 1; gap--) {
      ...
      for (const p of providers) {
        ...
        // Use stored coding_score when available (fetched from API at provider insertion),
        // fallback to static getCodingScore
        const score = typeof p.coding_score === "number" && Number.isFinite(p.coding_score)
          ? p.coding_score
          : getCodingScore(modelId);
...
function extractLastUserMessage(messages) {
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    if (msg && (msg.role === "user" || msg.role === "human")) {
      ...
      if (typeof content === "string") return content;
      if (Array.isArray(content)) {
        // If the last user message contains tool_result blocks, it's an autonomous LLM request
        // Return null to prevent intent tracking on tool outputs
        const hasToolResult = content.some((c) => c && c.type === "tool_result");
        if (hasToolResult) return null;
        const textBlock = content.find((c) => c && c.type === "text" && c.text);
        if (textBlock) return textBlock.text;
      }
      // Found a user message but couldn't extract text
      return null;
    }
  }
  return null;
