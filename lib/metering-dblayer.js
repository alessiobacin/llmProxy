...
function resolveDbLayerUrl() {
  const url = process.env.DBLAYER_URL;
  if (url) return url.replace(/\/$/, "");
  const mode = process.env.LLMPROXY_ENV || process.env.NODE_ENV;
  if (mode === "staging") return "http://localhost:6046";
  if (mode === "production") return "http://localhost:7046";
 ...
 * @returns {object}  sink
 */
function createDbLayerSink({ url, fallbackSink, fetchFn, notifier, healthCheckIntervalMs } = {}) {
  ...
  async function probe() {
    ...
    try {
      ...
      clear();
      _available = r.ok;
      if (!r.ok) error = new Error(`health check returned ${r.status}`);
    } catch (err) {
      _available = false;
      error = err;
    }
    ...
    if (_notifier) {
      if (wasAvailable && !_available) {
        _notifier.notifyUnreachable("db-layer", baseUrl, error);
      } else if (!wasAvailable && _available) {
        _notifier.notifyRecovered("db-layer", baseUrl);
      }
    }
  }
