...
// Per-model intelligence scores (0-100) for intent escalation routing.
// Higher = smarter model. Defaults by tier if not explicitly listed.
const INTELLIGENCE_SCORES = {
  // Economy (~40-65)
  "deepseek-chat": 50,
  "deepseek-v4-flash": 56.2,
  "deepseek-v4-flash-free": 50,
  "deepseek-v4-pro": 57,
  "kimi-k2.6": 60,
  "kimi-k3": 76.2,
  "qwen3.7-max": 66,
...
function getIntelligenceScore(modelName) {
  const lower = String(modelName || "").trim().toLowerCase();
  if (INTELLIGENCE_SCORES[lower] !== undefined) return INTELLIGENCE_SCORES[lower];
  // Fallback by tier
  const tier = getTierForModel(modelName);
