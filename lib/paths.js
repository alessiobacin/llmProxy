...
function getDockerHostProjectsRoot(options = {}) {
  const env = options.env || process.env;
  const homeDir = options.homeDir || os.homedir();
  const platform = options.platform || process.platform;
  const explicit = String(env.LLMPROXY_HOST_PROJECTS_ROOT || "").trim();
...
function createPaths(options = {}) {
  ...
  return {
    ...
    logsDir,
    serviceDir,
    serviceConfigFile: path.join(serviceDir, "config.json"),
    foregroundRunPidFile: path.join(serviceDir, "foreground-run.json"),
    serviceRunnerFile: platform === "win32" ? path.join(serviceDir, "service-runner.cmd") : undefined,
    ...
    providerRegistryFile: path.join(dataRoot, "provider-registry.json"),
    ...
    meteringFile: path.join(logsDir, "metering.jsonl"),
