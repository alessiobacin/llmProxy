...
// tenant_id optional (required if client_id is present).
// client_id optional.
// Valid combinations:
//   1. master_company + project_id                              (MC direct project)
//   2. master_company + tenant_id + project_id                  (tenant project)
//   3. master_company + tenant_id + client_id + project_id      (tenant → client → project)
...
function validateHierarchyContextForBilling(hierarchyContext) {
  ...
  // client_id requires tenant_id
  if (hasField("client_id") && !hasField("tenant_id")) missingFields.push("tenant_id");
