...
function assertGlobalServicePortAccess({ port, env = process.env } = {}) {
  ...
  throw new Error(
    ...
    + "Non avviare il checkout locale in production su questa porta quando il servizio persistente e' installato. "
    + "Per lo sviluppo usa la porta dev 5045 (`llmproxy run`); per usare 7045 arresta prima il servizio persistente (`llmproxy service:stop`)."
  );
}
...
function listPortListeners({ port, execCommand = defaultExec, platform = process.platform } = {}) {
  ...
  if (platform === "win32") {
    const result = execCommand("C:\\Windows\\System32\\netstat.exe", ["-ano", "-p", "tcp"]);
    if (result?.error) {
      throw result.error;
    }
