...
function resolveClaudeProjectSettings(projectPath) {
  ...
  while (true) {
    ...
    if (fs.existsSync(settingsFile)) {
      ...
      const llmStatsApiKey = String(env.LLMPROXY_LLM_STATS_API_KEY || env.LLM_STATS_API_KEY || "").trim();
      const sendgridApiKey = String(env.LLMPROXY_SENDGRID_API_KEY || env.SENDGRID_API_KEY || "").trim();
      const sendgridFromEmail = String(env.LLMPROXY_SENDGRID_FROM_EMAIL || env.SENDGRID_FROM_EMAIL || "").trim();
      const sendgridToEmail = String(env.LLMPROXY_SENDGRID_TO_EMAIL || env.SENDGRID_TO_EMAIL || "").trim();
      const sendgridToMessageType = parseSendgridMessageTypes(env.LLMPROXY_SENDGRID_TO_MESSAGE_TYPE || env.SENDGRID_TO_MESSAGE_TYPE);
