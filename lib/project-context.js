  ...
  while (true) {
    ...
    if (fs.existsSync(settingsFile)) {
      ...
      const configuredModelLooksLikeFallbackList = configuredModel.includes(",")
        || /^(copilot|openrouter|zai|z\.ai|kimi|openai|anthropic|deepseek|groq|mistral|xai|perplexity|together|fireworks)[:\-]/i.test(configuredModel);
      if (authToken === "proxy-local" && configuredModel && configuredModelLooksLikeFallbackList) {
        return {
          configuredModel,
          configuredModelSource: "settings.json",
        };
      }
      if (authToken === "proxy-local" && claudeModel) {
        return {
