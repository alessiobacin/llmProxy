...
function createFilePersistence(filePath) {
  ...
  const sharedMode = ["1", "true", "yes", "on"].includes(String(process.env.LLMPROXY_SHARED_PROVIDER_REGISTRY || "").trim().toLowerCase());
  const fileMode = sharedMode ? 0o660 : 0o600;
  const dirMode = sharedMode ? 0o2775 : 0o700;
