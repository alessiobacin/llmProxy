...
function buildDefaultProbeFn(options = {}) {
  ...
  return async ({ provider, model, fetchFn }) => {
    ...
    // Keep provider:list benchmarks aligned with the runtime adapter. OpenCode
    // Go selects messages/chat/responses by model, so a fixed endpoint produces
    // misleading 400/500 benchmark errors even when inference succeeds.
    if (pType === "opencode-go") {
      ...
      try {
        ...
        return probeApiKeyProviderModel({
          provider,
        ...
        return { ok: false, error: e?.message || "probe-error" };
      }
    }
