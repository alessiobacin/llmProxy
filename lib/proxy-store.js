...
function normalizeProxyUrlWithFallbackUser(raw) {
  const normalized = normalizeProxyUrl(raw);
  try {
    const parsed = new URL(normalized);
    // Se l'username e' vuoto ma c'e' una password (formato :key@host), usa "proxy" come default
    if (!parsed.username && parsed.password) {
      parsed.username = "proxy";
      return parsed.href;
    }
  } catch {
    // ignora
...
function createProxyStore(options = {}) {
  ...
  function addProxy(url, name) {
    const normalizedUrl = normalizeProxyUrlWithFallbackUser(url);
    ...
    if (existing) {
      ...
      existing.name = name || existing.name || undefined;
