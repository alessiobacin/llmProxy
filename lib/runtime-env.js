...
function readClaudeGlobalEnv(homeDir, dataRoot) {
  try {
    // Try candidate paths in priority order
    const candidates = [];
    if (homeDir) candidates.push(path.resolve(String(homeDir)));
    candidates.push(os.homedir());
    // If dataRoot looks like a user profile path (e.g. .../Users/<user>/...),
    // derive the user's home from it as fallback for Windows services
    if (dataRoot) {
      const d = path.resolve(String(dataRoot));
      const userIdx = d.toLowerCase().indexOf(path.sep + "users" + path.sep);
      if (userIdx !== -1) {
        const after = d.slice(userIdx + 7); // skip "\Users\"
        const sepIdx = after.indexOf(path.sep);
        if (sepIdx !== -1) {
          // Derive user's home from dataRoot (e.g. C:\Users\<user>)
          candidates.push(d.slice(0, userIdx + 7 + sepIdx));
        }
      }
    }
    for (const base of candidates) {
      const settingsFile = path.join(base, ".claude", "settings.json");
...
function loadRuntimeEnv(options = {}) {
  ...
  const claudeGlobalEnv = readClaudeGlobalEnv(homeDir, dataRoot);
