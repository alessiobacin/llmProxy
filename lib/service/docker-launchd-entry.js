...
function ensureContainerUp() {
  ...
  if (up.status !== 0) {
    fail(`docker compose up failed: ${String(up.stderr || up.stdout || "unknown error").trim()}`);
    return false;
  }
  return true;
}
...
function stopContainer() {
  ...
  if (stop.status !== 0) {
    fail(`docker compose stop failed: ${String(stop.stderr || stop.stdout || "unknown error").trim()}`);
  }
}
