...
`;
  ...
  function isTransientResult(result) {
    ...
    return status === 5 && /Input\/output error/i.test(stderr);
  }

  function execWithRetry(args, maxAttempts = 3) {
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      const result = execLaunchctl(args);
      if (result.status === 0) return result;
      if (!isTransientResult(result)) return result;
      if (attempt < maxAttempts - 1) {
        // Wait before retry — give launchd time to settle
        const delaySec = (300 + attempt * 200) / 1000;
        spawnSync("sleep", [String(delaySec)], { encoding: "utf8" });
      }
    }
    return execLaunchctl(args);
  }

  function runBootstrap() {
    // bootstrap expects: launchctl bootstrap <domain> [service-path, ...]
    // domain = gui/<uid> (without label), service-file is the .plist path
    const bootstrapArgs = ["bootstrap", `gui/${process.getuid()}`, serviceFile];
    return execWithRetry(bootstrapArgs, 3);
  }
  ...
  function install() {
    ...
    // Bootout with retry for transient errors
    const bootoutResult = execWithRetry(["bootout", domainTarget], 3);
    ...
    // Wait for launchd to settle before bootstrap
  ...
  function start() {
    ...
    const startBootstrapResult = runBootstrap();
    if (startBootstrapResult.status !== 0 && !/already bootstrapped/i.test(String(startBootstrapResult.stderr || ""))) {
      return {
        ok: false,
        serviceFile,
        stdoutPath,
        stderrPath,
        stdout: startBootstrapResult.stdout,
        stderr: formatLaunchctlError(startBootstrapResult, "launchctl bootstrap failed"),
      };
    }
  ...
  function stop() {
    const result = execWithRetry(["bootout", domainTarget], 3);
  ...
  function status() {
    const result = execLaunchctl(["print", domainTarget]);
