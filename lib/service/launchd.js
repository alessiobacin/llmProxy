...
`;
  ...
  function isTransientBootstrapResult(result) {
    const stderr = String(result?.stderr || "");
    return Number(result?.status) === 5 && /Input\/output error/i.test(stderr);
  }

  function runBootstrap() {
    const bootstrapArgs = ["bootstrap", `gui/${process.getuid()}`, serviceFile];
    let result = execLaunchctl(bootstrapArgs);
    if (result.status !== 0 && isTransientBootstrapResult(result)) {
      result = execLaunchctl(bootstrapArgs);
    }
    return result;
  }
