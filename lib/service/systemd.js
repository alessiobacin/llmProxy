...
function createSystemdServiceManager(options = {}) {
  ...
  function resolveOriginalUserId() {
    if (Number.isInteger(options.userId)) return options.userId;
    const sudoUser = process.env.SUDO_USER;
    if (sudoUser) {
      try {
        const result = spawnSync("id", ["-u", sudoUser], { encoding: "utf8" });
        if (result.status === 0) return parseInt(result.stdout.trim(), 10);
      } catch { /* fall through */ }
    }
    return typeof process.getuid === "function" ? process.getuid() : null;
  }

  const userId = resolveOriginalUserId();
...
`;
  ...
  function execSystemctl(args) {
    ...
    // When running as root (e.g. sudo npm install -g), delegate to the real user
    // so systemctl --user can reach the correct D-Bus session bus.
    if (username && typeof process.getuid === "function" && process.getuid() === 0 && runtimeDir) {
  ...
  function waitForUserBus(maxWaitMs = 5000) {
    const busSocket = runtimeDir ? path.join(runtimeDir, "bus") : null;
    if (!busSocket) return { ok: false, waited: 0 };
    const start = Date.now();
    while (Date.now() - start < maxWaitMs) {
      if (fs.existsSync(busSocket)) return { ok: true, waited: Date.now() - start };
      // Bus not yet available — short sleep via spin-wait with tight timeout
      const deadline = Date.now() + 200;
      while (Date.now() < deadline) { /* spin */ }
    }
    return { ok: fs.existsSync(busSocket), waited: Date.now() - start };
  }
