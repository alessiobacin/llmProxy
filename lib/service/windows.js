...
function createWindowsServiceManager(options = {}) {
  ...
  function stop() {
    ...
    return {
      ok: result.status === 0,
      stdout: result.stdout,
      stderr: result.status === 0 ? "" : (result.stderr || result.stdout || "sc.exe stop failed"),
    };
  }
  ...
  function status() {
    ...
    const active = combined.includes("running") || /\b(stato|state)\b[\s:]+4\b/i.test(`${stdout}\n${stderr}`);
    const installed = result.status === 0 || combined.includes("state") || combined.includes("stato") || combined.includes("running") || combined.includes("stopped");
    return {
      ok: installed,
      active,
      stdout: stdout,
