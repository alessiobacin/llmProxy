...
exports.V11_OPERATOR_ROLES = exports.V11_ADMIN_ROLES = exports.VALID_SCOPE_TYPES = void 0;
...
function normalizeAuthHeaderToken(value) {
    const text = String(value ?? "").trim();
    if (!text)
        return "";
...
function parseLocalScopeToken(req) {
    if (!req)
        return null;
    ...
    if (!token.startsWith("llmproxy-local-user:"))
        return null;
    ...
    if (parts.length >= 3) {
        if (!secret)
            return null;
        const expected = node_crypto_1.default.createHmac("sha256", secret).update(userId).digest("hex");
        if (parts[2] !== expected)
            return null;
    }
    return {
...
function parseHierarchyContext(req) {
    ...
    if (!raw || typeof raw !== "object")
        return parseLocalScopeToken(req);
