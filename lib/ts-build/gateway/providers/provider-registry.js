...
function createProviderRegistry(options) {
    ...
    function collectHierarchyCandidates(store, hierarchyContext) {
        ...
        for (const match of matches) {
            ...
            const found = store.entries.filter((entry) => {
                if (entry.scope_type !== match.scope_type)
                    return false;
    ...
    function resolveCandidates(hierarchyContext, requestedProvider) {
        const store = persistence.read();
