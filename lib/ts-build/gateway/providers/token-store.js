...
function extractProviderAccessToken(provider) {
    const directToken = String(provider?.access_token ?? provider?.api_key ?? "").trim();
    if (directToken)
        return directToken;
    const nestedToken = String(provider?.credentials?.access_token ?? provider?.credentials?.api_key ?? "").trim();
    if (nestedToken)
        return nestedToken;
    return "";
}
function normalizeProvider(provider, index) {
    ...
    const accessToken = extractProviderAccessToken(provider);
    const token = {
        id,
