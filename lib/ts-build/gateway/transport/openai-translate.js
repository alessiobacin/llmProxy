...
exports.translateResponse = exports.translateRequest = exports.isClaude = exports.resolveSupportedModel = exports.mapModel = exports.getAvailableModels = exports.msgId = exports.DEFAULT_COPILOT_MODEL = void 0;
...
function formatContent(blocks) {
    ...
    // Debug: log when images are detected
    if (hasImages && process.env.DEBUG_VISION) {
        console.error("[formatContent] Images detected:", blocks.filter(isImage).length);
    }
    if (hasImages) {
        const result = [];
...
function translateResponse(openai, responseModel) {
    ...
    if (process.env.DEBUG_VISION) {
        console.error("[translateResponse] reasoning_content present:", !!reasoningContent, "length:", reasoningContent?.length || 0);
    }
    const result = {
        id: msgId(),
        type: "message",
