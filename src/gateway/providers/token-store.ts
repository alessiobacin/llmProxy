...
interface TokenStore {
  filePath: string;
  load: () => ProviderToken | null;
  save: (data: Record<string, unknown>) => ProviderToken;
  clear: () => void;
  listProviders: () => ProviderToken[];
  getProvider: (providerId: string) => ProviderToken | null;
  saveProvider: (providerId: string, data: Record<string, unknown>, metadata?: { name?: string }) => ProviderToken;
  setProviderOrder: (providerIds: string[]) => ProviderToken[];
...
function normalizeRegistry(data: Record<string, unknown> | null): ProviderRegistry {
  ...
  const rawProviders = Array.isArray(data?.providers) ? data.providers as Record<string, unknown>[] : [];
  const providers: ProviderToken[] = [];
  const providerMap = new Map<string, ProviderToken>();
  ...
  const rawOrder: string[] = Array.isArray(data?.order)
    ? (data.order as unknown[]).map((id) => normalizeProviderId(id)).filter((id) => id.length > 0)
    : [];
...
function createTokenStore(options: { filePath?: string; persistence?: FilePersistence } = {}): TokenStore {
  ...
  function load(): ProviderToken | null {
    return listProviders()[0] || null;
  }

  function save(data: Record<string, unknown>): ProviderToken {
  ...
  function listProviders(): ProviderToken[] {
    return loadRegistry().providers;
  }

  function getProvider(providerId: string): ProviderToken | null {
    const targetId = normalizeProviderId(providerId);
    return listProviders().find((p) => p.id === targetId) || null;
  }

  function saveProvider(providerId: string, data: Record<string, unknown>, metadata: { name?: string } = {}): ProviderToken {
    const targetId = normalizeProviderId(providerId);
  ...
  function setProviderOrder(providerIds: string[]): ProviderToken[] {
    const registry = loadRegistry();
    ...
    for (const pid of normalizedIds) {
      if (registry.providers.some((p) => p.id === pid) && !nextOrder.includes(pid)) nextOrder.push(pid);
    }
  ...
  function moveProvider(providerId: string, position: number): ProviderToken[] {
    const targetId = normalizeProviderId(providerId);
  ...
  function clearProvider(providerId: string): ProviderToken[] | null {
    const targetId = normalizeProviderId(providerId);
    const registry = loadRegistry();
    registry.providers = registry.providers.filter((p) => p.id !== targetId);
    registry.order = registry.order.filter((id) => id !== targetId);
  ...
  function renameProvider(providerId: string, nextName: string): ProviderToken {
    const targetId = normalizeProviderId(providerId);
...
export type { TokenStore, ProviderToken };
