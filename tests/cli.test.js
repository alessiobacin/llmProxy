...
test("vscode-chat:setup writes the proxy config to standard and Insiders profiles when both are present", async () => {
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "llmproxy-vscode-home-"));
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "llmproxy-vscode-project-"));
  const stdout = createWritableBuffer();
  const stderr = createWritableBuffer();

  const stableDir = path.join(tempHome, "Library", "Application Support", "Code", "User");
  const insidersDir = path.join(tempHome, "Library", "Application Support", "Code - Insiders", "User");
  ...
  const stableFile = path.join(stableDir, "chatLanguageModels.json");
  const insidersFile = path.join(insidersDir, "chatLanguageModels.json");
  assert.ok(fs.existsSync(stableFile));
  assert.ok(fs.existsSync(insidersFile));

  const stableConfig = JSON.parse(fs.readFileSync(stableFile, "utf8"));
  const insidersConfig = JSON.parse(fs.readFileSync(insidersFile, "utf8"));
  assert.ok(Array.isArray(stableConfig));
  assert.ok(Array.isArray(insidersConfig));
  for (const config of [stableConfig, insidersConfig]) {
