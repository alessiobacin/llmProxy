...
test("selectEscalatedProviderIndex returns next index when providers[0] is free but ALL providers are free", () => {
  ...
  const providers = [
    { id: "nvidia-glm",    free_model: true },
    { id: "nvidia-m3",     free_model: true },
    { id: "nvidia-nemotron", free_model: true },
  ];
  assert.equal(isFreeModelProvider(providers[0]), true);
  // Tutti free: l'escalation deve poter selezionare il livello 1.
...
test("auto-escalation guard: skip when current provider is free AND a non-free candidate exists downstream", () => {
  // Caso "vecchio comportamento voluto": non vogliamo sprecare un paid quando
  // un provider free ha appena risposto. La guardia deve restare attiva
  // SE esiste almeno un provider non-free tra i candidati.
  const providers = [
    { id: "free-tier",      free_model: true },
    { id: "paid-backup",    free_model: false },
  ];
  const currentIsFree = providers[0].free_model === true;
  const someCandidateIsPaid = providers.some((p) => p.free_model !== true);
  // Logica attesa del guard: skip solo se (currentIsFree && someCandidateIsPaid)
  const shouldSkipOldLogic = !currentIsFree; // currenta buggy nel codice
  const shouldSkipCorrectLogic = currentIsFree && someCandidateIsPaid;
  assert.notEqual(shouldSkipOldLogic, shouldSkipCorrectLogic, "logiche devono divergere");
});

test("auto-escalation guard: enable when every candidate downstream is free (no paid tier to protect)", () => {
  const providers = [
    { id: "free-1", free_model: true },
    { id: "free-2", free_model: true },
    { id: "free-3", free_model: true },
  ];
  const currentIsFree = providers[0].free_model === true;
  const someCandidateIsPaid = providers.some((p) => p.free_model !== true);
  const skipEscalation = currentIsFree && someCandidateIsPaid;
  assert.equal(skipEscalation, false, "con tutti free, escalation deve partire");
});
